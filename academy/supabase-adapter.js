...
(function(){
  ...
  async function request(path,options={}){if(!config.projectUrl||!config.publishableKey)throw new Error('Supabase-Projekt-URL oder Publishable Key fehlt.');activityHandler('start');try{const res=await fetch(config.projectUrl+path,options);const text=await res.text();let body=text;try{body=text?JSON.parse(text):null}catch{}if(!res.ok)throw new Error(body?.msg||body?.message||body?.error_description||body?.error||`HTTP ${res.status}`);return body}finally{activityHandler('end')}}
