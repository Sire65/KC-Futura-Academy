...
const indexedLed=document.querySelector('#indexedLed'),indexedTrafficLed=document.querySelector('#indexedTrafficLed'),supabaseLed=document.querySelector('#supabaseLed'),supabaseTrafficLed=document.querySelector('#supabaseTrafficLed');
const settingsBtn=document.querySelector('#settingsBtn');
const exitBtn=document.querySelector('#exitBtn');
const settingsModal=document.querySelector('#settingsModal');
const voiceMonitor=document.querySelector('#voiceMonitor');
const voiceMonitorText=document.querySelector('#voiceMonitorText');
let academyAudioContext=null;
function setVoiceMonitor(mode='ready',label=''){
  if(!voiceMonitor)return;
  voiceMonitor.classList.remove('is-ready','is-speaking','is-pending','is-off','is-error');
...
async function unlockAcademyAudio(){
  try{
    ...
    if(Ctx){academyAudioContext=academyAudioContext||new Ctx();if(academyAudioContext.state==='suspended')await academyAudioContext.resume()}
    if(window.speechSynthesis){speechSynthesis.resume();speechSynthesis.getVoices()}
    return true;
  }catch{return false}
}
let academySpeechPrimed=false;
function primeAcademySpeech(){
...
async function playAcademyTestTone(){
  const ok=await unlockAcademyAudio();if(!ok||!academyAudioContext)throw new Error('AudioContext nicht verfügbar');
  return new Promise(resolve=>{const osc=academyAudioContext.createOscillator(),gain=academyAudioContext.createGain();osc.type='sine';osc.frequency.value=660;gain.gain.setValueAtTime(.0001,academyAudioContext.currentTime);gain.gain.exponentialRampToValueAtTime(.09,academyAudioContext.currentTime+.03);gain.gain.exponentialRampToValueAtTime(.0001,academyAudioContext.currentTime+.28);osc.connect(gain).connect(academyAudioContext.destination);osc.start();osc.stop(academyAudioContext.currentTime+.3);osc.onended=resolve})
}
async function runAcademyAudioTuv(){
  const status=document.querySelector('#audioTuvStatus'),btn=document.querySelector('#runAudioTuv');if(!status)return;
  btn&&(btn.disabled=true);status.className='settings-note';status.textContent='Prüfung läuft: Testton und Testansage …';
  try{
    state.sound=true;soundBtn.textContent='🔊';document.querySelector('#modalSound').checked=true;saveSettings();
    setVoiceMonitor('pending','Audio-TÜV läuft');await playAcademyTestTone();
    const result=await speak('Dies ist ein Audiotest der KC FUTURA Academy.',{characterKey:state.coach});
    if(result?.error||result?.silent)throw new Error('Sprachausgabe konnte nicht gestartet werden');
    status.className='settings-note tuv-pass';status.textContent='BESTANDEN: Testton und Testansage wurden gestartet. Bitte bestätigen Sie zusätzlich, dass beides hörbar war.';setVoiceMonitor('ready','Ton geprüft');
  }catch(err){status.className='settings-note tuv-fail';status.textContent='NICHT BESTANDEN: '+(err?.message||'Audioausgabe nicht verfügbar');setVoiceMonitor('error','Tonfehler')}finally{btn&&(btn.disabled=false)}
}

const exitModal=document.querySelector('#exitModal');
const dashboardModal=document.querySelector('#dashboardModal');
const roadmapModal=document.querySelector('#roadmapModal');
...
const get=(store,key,f='')=>{try{if(store==='localStorage'||store==='sessionStorage'){flashLed(indexedTrafficLed);return window.KCSecureStorage?.getItem(key)??f}return window[store]?.getItem(key)??f}catch{return f}};
const set=(store,key,v)=>{try{if(store==='localStorage'||store==='sessionStorage'){flashLed(indexedTrafficLed);window.KCSecureStorage?.setItem(key,v);return true}window[store]?.setItem(key,v);return true}catch{return false}};
const saved=safeParse(get('localStorage','kcAcademySettings','{}'),{});
...
const SCENE_CORE_VERSION='1.0.0';
const DIALOGUE_CORE_VERSION='1.0.0';
const TELEMETRY_KEY='kcAcademyTelemetryV1';
const PARTICIPANT_PROFILE_KEY='kcAcademyParticipantProfileV1';
const PARTICIPANT_PROFILE_VERSION='1.0.0';
let participantProfile=safeParse(get('localStorage',PARTICIPANT_PROFILE_KEY,'null'),null);
let pendingParticipantName='';
function participantCode(){const words=['KALLE','MARC','LAURA','FUTURA','WERNE','KOCH'];return `${words[Math.floor(Math.random()*words.length)]}-${Math.floor(1000+Math.random()*9000)}`}
async function sha256Text(value){const data=new TextEncoder().encode(String(value||''));const hash=await crypto.subtle.digest('SHA-256',data);return [...new Uint8Array(hash)].map(b=>b.toString(16).padStart(2,'0')).join('')}
function saveParticipantProfile(profile){participantProfile={version:PARTICIPANT_PROFILE_VERSION,...profile,updatedAt:new Date().toISOString()};window.KCParticipantDataCore?.saveProfile?.(participantProfile).catch(()=>{});set('localStorage',PARTICIPANT_PROFILE_KEY,JSON.stringify(participantProfile));state.name=participantProfile.displayName;set('sessionStorage','kcAcademyName',state.name);window.KCSupabaseAdapter?.setParticipantProfile?.(participantProfile);window.KCSupabaseAdapter?.uploadSnapshot?.('participant_profile',participantProfile,{participantName:state.name,participantId:participantProfile.id}).catch(()=>{});return participantProfile}

const CHARACTER_OVERRIDE_KEY='kcAcademyCharacterOverridesV1';
const elevenConfig=safeParse(get('localStorage',ELEVEN_CONFIG_KEY,'{}'),{});
elevenConfig.endpoint=elevenConfig.endpoint||'';
elevenConfig.anonKey=elevenConfig.anonKey||'';
elevenConfig.modelId=elevenConfig.modelId||'eleven_multilingual_v2';
elevenConfig.outputFormat=elevenConfig.outputFormat||'mp3_44100_128';
elevenConfig.autoGenerate=Boolean(elevenConfig.autoGenerate);
elevenConfig.voiceIds=elevenConfig.voiceIds||{};
elevenConfig.providerSettings=elevenConfig.providerSettings||{};
elevenConfig.providerSettings.elevenlabs=elevenConfig.providerSettings.elevenlabs||{modelId:'eleven_multilingual_v2',outputFormat:'mp3_44100_128',instructions:''};
...
const SYSTEM_TEXT_KEY='kcAcademySystemTextsV1';
const defaultSystemTexts={
  welcome_laura:'Hallo {name}. Herzlich willkommen in der KC FUTURA Academy. Ich bin Laura und begleite dich durch die Weiterbildung. Marc und Laura geben dir Praxistipps, und unsere Gäste zeigen dir, wie dein Verhalten bei ihnen ankommt.',
  welcome_marc:'Hallo {name}. Herzlich willkommen in der KC FUTURA Academy. Ich bin Marc und begleite dich durch die Weiterbildung. Marc und Laura geben dir Praxistipps, und unsere Gäste zeigen dir, wie dein Verhalten bei ihnen ankommt.',
  ready_du:'Bist du bereit, {name}?',ready_sie:'Sind Sie bereit, {name}?',overview_button:'Zur Academy-Übersicht',
...
const CATEGORY_DEFS={
  reklamation:{label:'Reklamationsmanagement',icon:'⚠️'},
  service:{label:'Kundenservice & Gastfreundschaft',icon:'🤝'},
  konflikt:{label:'Konfliktmanagement',icon:'💬'},
  team:{label:'Teamarbeit & Zusammenarbeit',icon:'👥'},
  hygiene:{label:'Hygiene & Lebensmittelsicherheit',icon:'🧼'},
  allergene:{label:'Allergene & besondere Ernährungsformen',icon:'🌾'},
  jugend:{label:'Jugendschutz',icon:'🛡️'},
  organisation:{label:'Organisation & Risikomanagement',icon:'📋'},
  sicherheit:{label:'Sicherheit & besondere Situationen',icon:'🚨'},
  gemeinschaft:{label:'Köcheclub & Gemeinschaft',icon:'🎄'}
};
const MODULE_META={
  reklamation:{primary:'reklamation',categories:['reklamation','service'],figures:['herr_becker','frau_schmitt','lukas']},
  jugend:{primary:'jugend',categories:['jugend','sicherheit'],figures:['lukas']},
  konflikt:{primary:'konflikt',categories:['konflikt','team'],figures:['michael','sabrina']},
  rechenfehler:{primary:'organisation',categories:['organisation','reklamation','service'],figures:['michael','sabrina']},
  gas:{primary:'organisation',categories:['organisation','team','sicherheit'],figures:['michael','sabrina','dr_brinkmann']},
  fundsache:{primary:'organisation',categories:['organisation','gemeinschaft'],figures:['frau_schmitt','leo','michael']},
  mettwurst:{primary:'service',categories:['service','team'],figures:['herr_becker','michael','sabrina']},
  probierschluck:{primary:'service',categories:['service'],figures:['sabrina']},
  hund:{primary:'hygiene',categories:['hygiene','organisation','sicherheit'],figures:['dr_brinkmann','michael','sabrina']},
  allergene:{primary:'allergene',categories:['allergene','hygiene'],figures:['frau_schmitt','leo','dr_brinkmann']},
  hygiene:{primary:'hygiene',categories:['hygiene','sicherheit'],figures:['dr_brinkmann','michael','sabrina']},
  vegetarisch:{primary:'allergene',categories:['allergene','service'],figures:['michael','sabrina']},
  wechselgeld:{primary:'organisation',categories:['organisation','reklamation'],figures:['michael','sabrina','herr_becker']},
  geldboerse:{primary:'service',categories:['service','organisation'],figures:['frau_schmitt','michael','sabrina']},
  leo:{primary:'team',categories:['team','gemeinschaft'],figures:['leo','michael','sabrina']},
  kind:{primary:'sicherheit',categories:['sicherheit','team','service'],figures:['michael','sabrina']},
  nachbarstand:{primary:'konflikt',categories:['konflikt','gemeinschaft','service'],figures:['michael','sabrina','herr_koslowski']},
  anlieferung:{primary:'organisation',categories:['organisation','sicherheit','team'],figures:['eugen','michael','sabrina']},
  ...
  freundliches_wort:{primary:'service',categories:['service','team','gemeinschaft'],figures:['hannes','leo','laura']},
  zwei_euro:{primary:'organisation',categories:['organisation','team'],figures:['detlef','leo','marc','laura']},
...
function saveBrowsePrefs(){saved.episodeView=state.episodeView;set('localStorage','kcAcademySettings',JSON.stringify({...saved,coach:state.coach,address:state.address,voice:state.voice,speechRate:state.speechRate,theme:state.theme,fontScale:state.fontScale,reduceMotion:state.reduceMotion,voiceProvider:state.voiceProvider,subtitles:state.subtitles,sound:state.sound,volume:state.volume,shuffleAnswers:state.shuffleAnswers,countdownEnabled:state.countdownEnabled,autoContinue:state.autoContinue,skipIntroductions:state.skipIntroductions,openLast:state.openLast,episodeView:state.episodeView}))}
function moduleProgress(id,m){if(state.completed[id])return 100;const q=Math.max(1,moduleStats(m).questionCount||m.scenes.filter(s=>s.question).length);const rows=TelemetryCore.read().filter(x=>x.module===id&&x.type==='answer');return Math.min(90,Math.round((rows.length/q)*100))}
...
const characterOverrides=safeParse(get('localStorage',CHARACTER_OVERRIDE_KEY,'{}'),{});
Object.entries(characterOverrides).forEach(([key,value])=>{if(chars[key])Object.assign(chars[key],value)});
...
const TelemetryCore={
  ...
  summary(){
    ...
    const byModule={};moduleIds.forEach(id=>{const a=answers.filter(x=>x.module===id);const f=finishes.filter(x=>x.module===id);const st=starts.filter(x=>x.module===id);byModule[id]={answers:a.length,correct:a.filter(x=>x.correct).length,wrong:a.filter(x=>!x.correct).length,runs:st.length,minutes:Math.round(f.reduce((n,x)=>n+(Number(x.durationMs)||0),0)/6000)/10,completed:Boolean(state.completed[id])}});
    return {rows,answers,finishes,starts,byModule,totalCorrect:answers.filter(x=>x.correct).length,totalWrong:answers.filter(x=>!x.correct).length,totalMinutes:Math.round(finishes.reduce((n,x)=>n+(Number(x.durationMs)||0),0)/6000)/10};
  }
};

const DialogueCore={
  version:DIALOGUE_CORE_VERSION,
  async play(lines,{onLine}={}){
    for(let i=0;i<lines.length;i++){
      if(state.paused)break;
      const row=lines[i];if(onLine)onLine(row,i);
      await speak(row.text,{highlight:row.highlight||null,characterKey:row.character||state.coach,audioKey:row.audioKey||''});
    }
  }
};

const SceneCore={
  version:SCENE_CORE_VERSION,
  normalize(id,m){
  ...
  validate(){return this.catalog().map(m=>({id:m.id,ok:Boolean(m.title&&m.subtitle&&Array.isArray(m.cast)),sceneCount:m.sceneCount,questionCount:m.questionCount}))},
  export(){downloadJson('kc-futura-scene-manifest.json',{version:this.version,academyVersion:ACADEMY_VERSION,generatedAt:new Date().toISOString(),modules:this.catalog().map(({scenes,...m})=>m)})}
};
...
const modules={
  ...
  reklamation:{season:1,episode:2,status:'fertig',points:6,title:'Reklamationsmanagement',subtitle:'Aus einer Reklamation einen zufriedenen Gast machen.',learning:'ruhig reagieren, die Wirkung verstehen, passende Lösungen finden und den KC Bilderrechner als Unterstützung kennen',avatar:'herr_becker.png',cast:['laura','marc','herr_becker','frau_schmitt','lukas'],ready:true,scenes:[
    ...
    {who:'Lukas',key:'lukas',text:()=>`Ich habe einen Apfelpunsch bestellt und schon einen Schluck getrunken. Jetzt sehe ich rote Lippenstiftreste am Glas.`,question:'Wie sollte das Team reagieren?',choices:[['Das Glas nur kurz abwischen.','bad'],['Sich entschuldigen, das Glas sofort austauschen und das beanstandete Glas zum Spülen geben.','good'],['Erklären, dass der Lippenstift bestimmt vom Gast selbst stammt.','bad']],good:'Richtig. Eine hygienische Reklamation wird ohne Diskussion ernst genommen und sofort sauber gelöst.',guest:'So etwas kann passieren. Entscheidend war für mich, dass das Glas sofort ausgetauscht und meine Reklamation freundlich behandelt wurde.'},
    {who:'Marc',key:'marc',reklamationMedia:true,text:()=>`Der KC Bilderrechner unterstützt dich bei einer professionellen Reklamationsbearbeitung. Unter dem Warenkorb findest du den Button Reklamation.`,tip:`Im geöffneten Reklamationsfenster werden Artikel, Reklamationsgrund, Belegstatus und Speichern klar geführt. Den genauen Ablauf übst du in der KC Bilderrechner-Schulung.`,question:'Wo findest du im KC Bilderrechner die Funktion Reklamation?',choices:[['Unter dem Warenkorb über den Button Reklamation.','good'],['In der Kopfzeile hinter der Uhr.','bad'],['Nur im PC-Manager.','bad']],good:'Richtig. Die Funktion Reklamation befindet sich unter dem Warenkorb.',guest:'Eine klar geführte Reklamation hilft dem Team, ruhig, nachvollziehbar und einheitlich zu handeln.'},
    {who:'Herr Becker',key:'herr_becker',text:()=>`Ich glaube, ich habe statt Feuerzangenbowle einen normalen Glühwein bekommen.`,question:'Was ist jetzt der richtige erste Schritt?',choices:[['Bestellung und erfasste Positionen ruhig prüfen.','good'],['Dem Gast widersprechen.','bad'],['Die Reklamation ignorieren.','bad']],good:'Richtig. Erst prüfen, dann freundlich korrigieren.',guest:'Obwohl der Stand voll war, wurde meine Bestellung ruhig geprüft. Der Fehler war schnell behoben und ich komme gern wieder.'},
  ...
  jugend:{season:1,episode:3,status:'fertig',points:3,title:'Jugendschutz',subtitle:'Freundlich kontrollieren und gesetzliche Verantwortung übernehmen.',learning:'gesetzliche Altersgrenzen einordnen, freundlich nach dem Ausweis fragen und bei fehlendem Nachweis sicher handeln',avatar:'lukas.png',cast:['laura','marc','lukas'],ready:true,scenes:[
    ...
    {who:'Lukas',key:'lukas',text:()=>`Ich habe meinen Ausweis heute leider nicht dabei. Aber ich bin wirklich schon achtzehn.`,question:'Was ist ohne sicheren Altersnachweis richtig?',choices:[['Ausnahmsweise verkaufen.','bad'],['Keinen alkoholischen Artikel mit Rum abgeben und eine alkoholfreie Alternative anbieten.','good'],['Nur die Hälfte ausschenken.','bad']],good:'Richtig. Ohne verlässlichen Nachweis wird kein altersbeschränkter Artikel abgegeben.',guest:'Das ist zwar schade, aber ich verstehe die Entscheidung. Ein alkoholfreier Punsch ist für heute auch in Ordnung.'},
  ...
  finale:{season:1,episode:38,status:'fertig',points:4,title:'Ein Weihnachtsmarkt verbindet',subtitle:'Herr Becker und Frau Schmitt entdecken gemeinsame Interessen.',learning:'erkennen, wie Gastfreundschaft, Atmosphäre und menschliche Begegnungen den Weihnachtsmarkt prägen',avatar:'herr_becker.png',cast:['laura','marc','herr_becker','frau_schmitt','michael','sabrina'],ready:true,scenes:[
    ...
    {who:'Marc',key:'marc',text:()=>`Rezepte kann man lernen. Eine Kasse kann man bedienen. Aber Herzlichkeit kommt von den Menschen.`,tip:`Jedes Jahr machen wir den Weihnachtsmarkt ein bisschen besser – und menschlicher.`,next:'Academy-Staffel abschließen'}
  ]}
};
...
function ttsText(t){return String(t).replace(/KC FUTURA Academy/gi,'KC FUTURA Äkädemi').replace(/\bAcademy\b/gi,'Äkädemi')}
...
const AudioStore={
  dbName:'kc-futura-academy-audio',storeName:'recordings',db:null,
  open(){if(this.db)return Promise.resolve(this.db);return new Promise((resolve,reject)=>{const req=indexedDB.open(this.dbName,1);req.onupgradeneeded=()=>{const db=req.result;if(!db.objectStoreNames.contains(this.storeName))db.createObjectStore(this.storeName,{keyPath:'key'})};req.onsuccess=()=>{this.db=req.result;resolve(this.db)};req.onerror=()=>reject(req.error)})},
  async get(key){try{const db=await this.open();const row=await new Promise((resolve,reject)=>{const r=db.transaction(this.storeName,'readonly').objectStore(this.storeName).get(key);r.onsuccess=()=>resolve(r.result||null);r.onerror=()=>reject(r.error)});if(!row)return null;if(row.encrypted&&row.payload){const plain=await KCSecureStorage.decryptBytes(row.payload);return {...row,blob:new Blob([plain],{type:row.meta?.mime||'audio/webm'})}}return row}catch{return null}},
  async put(key,blob,meta={}){const db=await this.open();const bytes=await blob.arrayBuffer();const payload=await KCSecureStorage.encryptBytes(bytes,'VOICE_RECORDING');const row={key,encrypted:true,payload,meta:{...meta,mime:blob.type||meta.mime||'audio/webm'},updatedAt:new Date().toISOString()};return new Promise((resolve,reject)=>{const r=db.transaction(this.storeName,'readwrite').objectStore(this.storeName).put(row);r.onsuccess=()=>resolve(true);r.onerror=()=>reject(r.error)})},
  async remove(key){try{const db=await this.open();return await new Promise((resolve,reject)=>{const r=db.transaction(this.storeName,'readwrite').objectStore(this.storeName).delete(key);r.onsuccess=()=>resolve(true);r.onerror=()=>reject(r.error)})}catch{return false}}
};
...
function saveElevenConfig(){
  set('localStorage',ELEVEN_CONFIG_KEY,JSON.stringify(elevenConfig));
  window.KCVoiceProvider?.configure({...elevenConfig,provider:state.voiceProvider});
}
function voiceIdFor(character){const map=elevenConfig.voiceIds?.[state.voiceProvider]||elevenConfig.voiceIds||{};return String(map?.[character]||'').trim()}
async function voiceAudioState(dialog){
  if(!dialog)return {kind:'missing',label:'Kein Dialog ausgewählt'};
  const existing=await AudioStore.get(dialog.key);
  if(!existing)return {kind:'missing',label:'Noch keine feste Sprachdatei vorhanden'};
  const currentHash=await hashText(dialog.text);
  const storedHash=existing.meta?.textHash||await hashText(existing.meta?.text||'');
  return storedHash===currentHash?{kind:'current',label:'Stimme aktuell und gespeichert',existing}:{kind:'stale',label:'Text geändert – Stimme neu erzeugen',existing};
}
async function generateElevenAudio(dialog,{previewOnly=false}={}){
  if(!dialog)throw new Error('Kein Dialog ausgewählt.');
  const voiceId=voiceIdFor(dialog.character);
  const ps=elevenConfig.providerSettings?.[state.voiceProvider]||{};const blob=await window.KCVoiceProvider.generate({provider:state.voiceProvider,text:dialog.text,voiceId,modelId:ps.modelId||elevenConfig.modelId,outputFormat:ps.outputFormat||elevenConfig.outputFormat,instructions:ps.instructions||''});
  if(previewOnly){const url=URL.createObjectURL(blob);const audio=new Audio(url);audio.onended=()=>URL.revokeObjectURL(url);audio.onerror=()=>URL.revokeObjectURL(url);await audio.play();return blob}
  const textHash=await hashText(dialog.text);
  await AudioStore.put(dialog.key,blob,{character:dialog.character,text:dialog.text,textHash,provider:state.voiceProvider,voiceId,modelId:(elevenConfig.providerSettings?.[state.voiceProvider]?.modelId||elevenConfig.modelId),generatedAt:new Date().toISOString(),mime:blob.type||'audio/mpeg'});
  return blob;
}
async function autoRegenerateDialog(dialogKey){
  if(!['elevenlabs','openai','azure'].includes(state.voiceProvider)||!elevenConfig.autoGenerate||!window.KCVoiceProvider?.isConfigured())return;
  const dialog=buildDialogRegistry().find(x=>x.key===dialogKey);if(!dialog||!voiceIdFor(dialog.character))return;
  try{await generateElevenAudio(dialog)}catch(err){console.warn('Automatische Sprachaktualisierung nicht möglich',err)}
}
...
async function ensureSpeechReady(){
  ...
  await new Promise(resolve=>{
    ...
    const handler=()=>{clearTimeout(timer);speechSynthesis.removeEventListener?.('voiceschanged',handler);finish()};
...
const VoiceCore={
  ...
  token:0,
  pickVoice(characterKey){
    const voices=availableVoices();
    if(!voices.length)return null;
    const profile=voiceProfiles[characterKey]||voiceProfiles[state.coach]||voiceProfiles.laura;
    ...
    if(characterKey===state.coach){
      const idx=state.voice==='two'?1:0;
      const chosen=pool[Math.min(idx,pool.length-1)]||voices[0];
      this.lastCoachVoiceName=chosen?.name||'';
      return chosen;
    }
    let usable=pool;
    if(characterKey==='herr_becker'&&state.coach==='marc'&&this.lastCoachVoiceName){
  ...
  async speak(raw,{highlight=null,characterKey=null,audioKey=null}={}){
    this.stop();
    const token=this.token;
    const t=text(raw);state.lastSpeech=t;repeatBtn.hidden=false;
    if(state.paused||!state.sound){setVoiceMonitor('off',state.sound?'Pausiert':'Ton aus');return {silent:true}}
    await unlockAcademyAudio();setVoiceMonitor('pending',`${chars[characterKey||state.coach]?.name||'Stimme'} startet`);
    if(audioKey&&state.voiceProvider!=='system'){
      const savedAudio=await AudioStore.get(audioKey);
      if(token!==this.token||state.paused)return {stopped:true};
      if(savedAudio?.blob){
        return new Promise(resolve=>{
          const token=this.token;this.activeResolve=resolve;const url=URL.createObjectURL(savedAudio.blob);const audio=new Audio(url);this.activeAudio=audio;this.activeObjectUrl=url;
          const finish=result=>{highlight?.classList.remove('reading');if(this.activeResolve===resolve)this.activeResolve=null;if(this.activeAudio===audio)this.activeAudio=null;if(this.activeObjectUrl===url){URL.revokeObjectURL(url);this.activeObjectUrl=null}setVoiceMonitor(result?.error?'error':(state.sound?'ready':'off'),result?.error?'Tonfehler':(state.sound?'Ton bereit':'Ton aus'));resolve(result||{done:true,recorded:true})};
          audio.onplay=()=>{if(token===this.token&&!state.paused){highlight?.classList.add('reading');setVoiceMonitor('speaking',`${chars[characterKey||state.coach]?.name||'Stimme'} spricht`)}};audio.onended=()=>finish();audio.onerror=()=>finish({error:true});
          audio.play().catch(()=>finish({error:true}));
        });
      }
    }
    if(!('speechSynthesis'in window)){setVoiceMonitor('error','Keine Gerätestimme');return {silent:true}};
    await ensureSpeechReady();
    return new Promise(resolve=>{
      this.activeResolve=resolve;
      const key=characterKey||state.coach;
      const profile=voiceProfiles[key]||voiceProfiles[state.coach]||voiceProfiles.laura;
      let settled=false,started=false,retries=0,startWatch=null;
      const finish=result=>{
      ...
      const launch=()=>{
        ...
        try{speechSynthesis.resume()}catch{}
        const u=new SpeechSynthesisUtterance(`  ${ttsText(t)}`);
        ...
        // Safari/iPad meldet onstart nicht immer zuverlässig: laufende Sprache nicht vorschnell abbrechen.
...
function moduleStats(m){const x=SceneCore.normalize('',m);return {scenes:x.sceneCount,questions:x.questionCount,estimatedMinutes:x.estimatedMinutes,duration:`ca. ${x.estimatedMinutes} Minuten`,scope:`${x.sceneCount} Lernschritte · ${x.questionCount} Wissensfragen`}}
...
function updateSceneProgress(){
  ...
  const m=modules[state.module];sceneProgress.hidden=false;
  sceneProgress.innerHTML=m.scenes.map((_,i)=>`<button class="scene-dot ${i<state.step?'done':i===state.step?'active':''}" data-scene="${i}" title="Zu Szene ${i+1} wechseln" aria-label="Zu Szene ${i+1} wechseln">${i<state.step?'✓':i+1}</button>`).join('')+`<small>Szene ${state.step+1}/${m.scenes.length}</small>`;
  sceneProgress.querySelectorAll('[data-scene]').forEach(btn=>btn.onclick=()=>jumpToScene(Number(btn.dataset.scene)));
}
function updateTime(){
  const now=new Date();dateDisplay.textContent=now.toLocaleDateString('de-DE');clockDisplay.textContent=now.toLocaleTimeString('de-DE');
  const end=state.paused&&state.pausedAt?state.pausedAt:Date.now();const ms=Math.max(0,end-state.startTime-state.totalPaused);const sec=Math.floor(ms/1000);const h=String(Math.floor(sec/3600)).padStart(2,'0');const m=String(Math.floor(sec%3600/60)).padStart(2,'0');const s=String(sec%60).padStart(2,'0');elapsedDisplay.textContent=`Laufzeit ${h}:${m}:${s}`;
}
setInterval(updateTime,500);updateTime();
function jumpToScene(target){
  if(!state.module||!Number.isInteger(target))return;
  const m=modules[state.module];if(target<0||target>=m.scenes.length||target===state.step)return;
  const prompt=settingLabel(`Möchtest du wirklich zu Szene ${target+1} wechseln?`,`Möchten Sie wirklich zu Szene ${target+1} wechseln?`);
  if(!window.confirm(prompt))return;
  stopSpeech();state.paused=false;pauseBtn.textContent='⏸ Pause';state.step=target;showScene();
}
function togglePause(){
  if(!state.paused){
    ...
    document.body.classList.add('academy-paused');
    showPauseOverlay();
  }else{
    state.paused=false;state.totalPaused+=Date.now()-state.pausedAt;state.pausedAt=0;
    pauseBtn.textContent='⏸ Pause';document.body.classList.remove('academy-paused');
    document.querySelector('#pauseOverlay')?.remove();
    if(state.view==='scene'&&state.module){showScene();return}
    if(state.view==='personal'){showPersonalWelcome();return}
    if(state.view==='overview'&&state.module){overview(state.module);return}
    if(state.lastSpeech)speak(state.lastSpeech,{characterKey:currentSpeakerKey()});
  }
}
function currentSpeakerKey(){
  if(state.view==='scene'&&state.module){return modules[state.module].scenes[state.step]?.key||state.coach}
  return state.coach;
}
...
async function beginParticipant(name){const clean=String(name||'').trim();if(participantProfile?.displayName&&participantProfile.displayName.toLowerCase()===clean.toLowerCase()){state.name=participantProfile.displayName;set('sessionStorage','kcAcademyName',state.name);showPersonalWelcome();return}const matches=await checkParticipantName(clean);pendingParticipantName=clean;if(matches.length){openParticipantChoice(clean,matches);return}await createNewParticipant()}
...
function openFirstRun(){
  ...
  btn.onclick=async()=>{
    const name=input.value.trim();if(name.length<MIN_NAME_LENGTH){input.focus();return}
    state.address=document.querySelector('input[name="firstRunAddress"]:checked')?.value||'du';
    state.coach=document.querySelector('input[name="firstRunCoach"]:checked')?.value||'laura';
    saved.autoUpdateCheck=document.querySelector('#firstRunUpdateCheck')?.checked!==false;
    saveSettings();
  ...
  refresh();setTimeout(()=>input.focus(),80)
}
function welcome(){if(participantProfile?.displayName){state.name=participantProfile.displayName;set('sessionStorage','kcAcademyName',state.name)}stopSpeech();setView('welcome');repeatBtn.hidden=true;setProgress(0,1,'Willkommen');
  const c=coach();screen.innerHTML=`<section class="panel futura-start">
  <aside class="side-panel"><p class="side-title">Hauptcoach wählen</p>${['laura','marc'].map(k=>{const x=chars[k];return `<button class="coach-option ${state.coach===k?'selected':''}" data-coach="${k}"><img src="${AV+x.img}" alt="${x.name}"><span><strong>${x.name}</strong><small>${x.role}</small></span></button>`}).join('')}</aside>
  <section class="main-stage"><div class="stage-brand"><img src="assets/kc_logo.webp" alt="Köcheclub Werne"><div><b>KC FUTURA Academy</b><br><small>Interaktive Weiterbildung</small></div></div><span class="side-title">Aus der Praxis – für die Praxis</span><h2>Willkommen in der Academy</h2><p class="lead">Kurze, lebendige Geschichten vom Weihnachtsmarkt – mit Entscheidungen, Rückmeldungen und echten Situationen.</p>
  <div class="identity-row"><label class="field">${settingLabel('Dein Vorname','Ihr Vorname')}<input id="nameInput" value="${esc(state.name)}" placeholder="z. B. Hans" autocomplete="given-name" maxlength="40"></label><div><span class="side-title">Anrede</span><div class="address-row"><label><input type="radio" name="address" value="du" ${state.address==='du'?'checked':''}> Du</label><label><input type="radio" name="address" value="sie" ${state.address==='sie'?'checked':''}> Sie</label></div></div></div>
  <p id="nameHint" class="name-hint">${settingLabel('Ohne deinen Vornamen kann die Academy nicht starten. Bitte Namen eingeben.','Ohne Ihren Vornamen kann die Academy nicht starten. Bitte Namen eingeben.')}</p><div class="start-actions"><button id="enter" class="primary" disabled>Academy starten</button></div>
  <div class="coach-summary"><img id="coachSummaryImg" src="${AV+c.img}" alt="${c.name}"><div><b id="coachSummaryName">${c.name}</b><span>${settingLabel('begleitet dich durch die Academy','begleitet Sie durch die Academy')}</span></div></div>
  <p><b>„Jedes Jahr machen wir den Weihnachtsmarkt ein bisschen besser – und menschlicher.“</b></p></section>
  <aside class="side-panel right"><p class="side-title">Einstellungen</p><div class="settings-stack"><div class="settings-card"><strong>Stimme</strong><select id="voiceSelect" class="compact-select"><option value="default" ${state.voice==='default'?'selected':''}>Stimme 1</option><option value="two" ${state.voice==='two'?'selected':''}>Stimme 2</option></select><button id="voicePreview" class="mini-action">▶ Stimme testen</button></div><div class="settings-card"><label class="setting-line"><span>Untertitel</span><input id="subtitleToggle" type="checkbox" ${state.subtitles?'checked':''}></label><label class="setting-line"><span>Ton</span><input id="soundToggle" type="checkbox" ${state.sound?'checked':''}></label></div></div></aside></section>`;
  const input=document.querySelector('#nameInput'),enter=document.querySelector('#enter'),hint=document.querySelector('#nameHint');
  function refresh(){const ok=input.value.trim().length>=MIN_NAME_LENGTH;enter.disabled=!ok;enter.classList.toggle('pulse',ok);hint.textContent=ok?settingLabel('Alles bereit – du kannst starten.','Alles bereit – Sie können starten.'):settingLabel('Ohne deinen Vornamen kann die Academy nicht starten. Bitte Namen eingeben.','Ohne Ihren Vornamen kann die Academy nicht starten. Bitte Namen eingeben.');hint.classList.toggle('ready',ok);return ok}
  async function start(){if(!refresh()){input.focus();return}await beginParticipant(input.value.trim())}
...
function showPersonalWelcome(){stopSpeech();setView('personal');setProgress(0,1,'Persönliche Begrüßung');const c=coach();const t=resolveSystemText(state.coach==='marc'?'welcome_marc':'welcome_laura');screen.innerHTML=`<section class="panel scene-panel"><div class="scene-grid"><img class="scene-portrait" src="${AV+c.img}" alt="${c.name}"><div class="scene-center"><div class="speaker">${c.name}</div>${state.subtitles?`<div class="speech" id="welcomeSpeech">${esc(text(t))}</div>`:''}<p class="lead">${esc(state.address==='sie'?resolveSystemText('ready_sie'):resolveSystemText('ready_du'))}</p><div class="actions"><button id="repeatGreeting" class="secondary">↻ Begrüßung wiederholen</button><button id="toAcademy" class="primary pulse">${esc(resolveSystemText('overview_button'))}</button></div></div><aside class="coach-response"><div class="coach-response-head"><img src="${AV+chars.marc.img}" alt="Marc"><div><b>Marc</b><small>Praxiscoach</small></div></div><p>„Aus Wissen wird Sicherheit – und aus Sicherheit guter Service.“</p></aside></div></section>`;speak(t,{highlight:document.querySelector('#welcomeSpeech'),characterKey:state.coach,audioKey:`coach:${state.coach}:personal-welcome`});document.querySelector('#repeatGreeting').onclick=()=>speak(t,{highlight:document.querySelector('#welcomeSpeech'),characterKey:state.coach,audioKey:`coach:${state.coach}:personal-welcome`});document.querySelector('#toAcademy').onclick=home}

async function introduceCharacter(key){
...
function home(){
  stopSpeech();setView('home');repeatBtn.hidden=true;setProgress(0,1,'Academy-Auswahl');
  const charOpen=!state.figuresCollapsed&&!state.skipIntroductions;
  const f=state.episodeFilters;
  const allEntries=Object.entries(modules).sort((a,b)=>(a[1].episode||99)-(b[1].episode||99));
  const filtered=allEntries.filter(([id,m])=>{
    const meta=metaFor(id),stats=moduleStats(m),hay=`${m.title} ${m.subtitle} ${m.learning||''} Folge ${m.episode}`.toLowerCase();
    if(f.search){const needle=f.search.toLowerCase().trim();const words=hay.split(/[^a-z0-9äöüß]+/).filter(Boolean);if((needle.length<=3?!words.includes(needle):!hay.includes(needle)))return false;}
    if(f.category!=='all'&&!meta.categories.includes(f.category))return false;
    if(f.figure!=='all'&&!(m.cast||meta.figures).includes(f.figure))return false;
    if(f.status==='completed'&&!state.completed[id])return false;
    if(f.status==='open'&&state.completed[id])return false;
    if(f.status==='ready'&&!m.ready)return false;
    if(f.duration==='short'&&stats.estimatedMinutes>5)return false;
    if(f.duration==='medium'&&(stats.estimatedMinutes<=5||stats.estimatedMinutes>10))return false;
    if(f.duration==='long'&&stats.estimatedMinutes<=10)return false;
    return true;
  });
  const cardHtml=([id,m])=>{const runs=moduleRunCount(id),progress=moduleProgress(id,m),done=Boolean(state.completed[id]);return `<article class="card episode-card ${m.ready?'ready':'locked'}" title="${esc(m.title)} – ${esc(m.subtitle)}" ${m.ready?`data-open-module="${id}" tabindex="0" role="button"`:''}><div class="card-topline"><span class="episode-number">Folge ${m.episode}</span><span class="card-top-right"><span class="episode-views">👁 ${runs}×</span>${done?'<span class="episode-done" title="Abgeschlossen">✅</span>':''}</span></div><div class="episode-card-main"><div class="card-media"><img src="${AV+m.avatar}" alt=""><button class="episode-play ${done?'repeat':''}" data-module="${id}" ${m.ready?'':'disabled'} aria-label="${done?'Folge wiederholen':'Folge abspielen'}" title="${done?'Folge wiederholen':'Folge starten'}">${done?'↻':'▶'}</button><span class="card-duration">${moduleStats(m).duration}</span><div class="card-progress" aria-label="Fortschritt ${progress} Prozent"><i style="width:${progress}%"></i></div></div><div class="card-body"><h3>${m.title}</h3><p>${m.subtitle}</p><div class="category-chips">${metaFor(id).categories.slice(0,2).map(k=>`<span>${CATEGORY_DEFS[k]?.label||k}</span>`).join('')}</div></div></div></article>`};
  let cards='';
  if(state.episodeView==='categories'){
    const groups={};filtered.forEach(e=>{const key=metaFor(e[0]).primary;(groups[key]||(groups[key]=[])).push(e)});
  ...
  document.querySelectorAll('[data-category-toggle]').forEach(btn=>btn.addEventListener('click',ev=>{ev.preventDefault();ev.stopPropagation();const key=btn.dataset.categoryToggle;if(state.categoryLocks[key])return;const section=btn.closest('.category-group');const content=section?.querySelector('.category-content');const closed=section?.classList.toggle('is-collapsed');if(content){content.hidden=Boolean(closed);content.style.display=closed?'none':''}btn.setAttribute('aria-expanded',String(!closed));const arrow=btn.querySelector('.category-arrow');if(arrow)arrow.textContent=closed?'▶':'▼'}));
  document.querySelectorAll('[data-category-lock]').forEach(btn=>btn.onclick=()=>{const key=btn.dataset.categoryLock;state.categoryLocks[key]=!state.categoryLocks[key];const section=btn.closest('.category-group');section?.classList.toggle('is-locked',state.categoryLocks[key]);if(state.categoryLocks[key]){section?.classList.remove('is-collapsed');const content=section?.querySelector('.category-content');if(content)content.hidden=false;const toggle=section?.querySelector('[data-category-toggle]');toggle?.setAttribute('aria-expanded','true');const arrow=toggle?.querySelector('.category-arrow');if(arrow)arrow.textContent='▼'}btn.textContent=state.categoryLocks[key]?'🔒':'🔓';btn.title=`Kategorie ${state.categoryLocks[key]?'entsperren':'offen halten'}`});
  document.querySelector('#filterToggle').onclick=()=>{state.filterOpen=!state.filterOpen;rerender()};
  const bindFilter=(id,key)=>{const el=document.querySelector(id);if(!el)return;el.onchange=()=>{state.episodeFilters[key]=el.value;rerender()}};
  const search=document.querySelector('#filterSearch');if(search){let timer;search.oninput=()=>{clearTimeout(timer);timer=setTimeout(()=>{state.episodeFilters.search=search.value.trim();rerender()},180)}}
  bindFilter('#filterCategory','category');bindFilter('#filterFigure','figure');bindFilter('#filterStatus','status');bindFilter('#filterDuration','duration');
  document.querySelector('#filterReset').onclick=()=>{state.episodeFilters={search:'',category:'all',figure:'all',status:'all',duration:'all'};rerender()};
  const toggle=document.querySelector('#toggleCharacters'),area=document.querySelector('#characterArea');toggle.onclick=()=>{if(state.figuresLocked)return;state.figuresCollapsed=!state.figuresCollapsed;toggle.setAttribute('aria-expanded',String(!state.figuresCollapsed));toggle.querySelector('span').textContent=state.figuresCollapsed?'▶':'▼';area.hidden=state.figuresCollapsed;document.querySelector('.character-collapsible').classList.toggle('is-open',!state.figuresCollapsed);document.querySelector('.character-collapsible').classList.toggle('is-closed',state.figuresCollapsed)};
  document.querySelector('#lockCharacters').onclick=e=>{state.figuresLocked=!state.figuresLocked;e.currentTarget.textContent=state.figuresLocked?'🔒':'🔓';e.currentTarget.title=`Figurenbereich ${state.figuresLocked?'entsperren':'sperren'}`};
  screen.querySelectorAll('[data-module]:not([disabled])').forEach(b=>b.onclick=e=>{e.stopPropagation();overview(b.dataset.module)});
  screen.querySelectorAll('[data-open-module]').forEach(card=>{card.onclick=e=>{if(e.target.closest('button'))return;overview(card.dataset.openModule)};card.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();overview(card.dataset.openModule)}}});
  screen.querySelectorAll('[data-character]').forEach(b=>b.onclick=()=>introduceCharacter(b.dataset.character));
...
function resetRecorderPreview(){if(recorderState.url){URL.revokeObjectURL(recorderState.url);recorderState.url=null}recorderState.blob=null;document.querySelector('#recordPlay').disabled=true;document.querySelector('#recordSave').disabled=true}
async function refreshVoiceStudio(){
  const characterSelect=document.querySelector('#voiceCharacter'),dialogSelect=document.querySelector('#voiceDialog'),textArea=document.querySelector('#voiceText');if(!characterSelect||!dialogSelect)return;
  const registry=buildDialogRegistry();if(!characterSelect.options.length)characterSelect.innerHTML=Object.entries(chars).map(([k,c])=>`<option value="${k}">${c.name} · ${c.role}</option>`).join('');
  const charKey=characterSelect.value||'laura';const dialogs=registry.filter(x=>x.character===charKey);const previous=dialogSelect.value;dialogSelect.innerHTML=dialogs.map(x=>`<option value="${x.key}">${esc(x.label)}</option>`).join('');
  const selected=dialogs.find(x=>x.key===previous)||dialogs[0];if(selected){dialogSelect.value=selected.key;textArea.value=selected.text;recorderState.dialogKey=selected.key}
  resetRecorderPreview();const audioState=await voiceAudioState(selected);const existing=audioState.existing||null;document.querySelector('#recordDelete').disabled=!existing;
  const status=document.querySelector('#recordingStatus');status.textContent=audioState.label;status.classList.remove('voice-stale','voice-current','voice-working');if(audioState.kind==='stale')status.classList.add('voice-stale');if(audioState.kind==='current')status.classList.add('voice-current');
  const panel=document.querySelector('#elevenLabsPanel');if(panel)panel.hidden=!['elevenlabs','openai','azure'].includes(state.voiceProvider);
  const voiceId=document.querySelector('#elevenVoiceId');if(voiceId)voiceId.value=voiceIdFor(charKey);
  const endpoint=document.querySelector('#elevenEndpoint');if(endpoint)endpoint.value=elevenConfig.endpoint;
  const anon=document.querySelector('#elevenAnonKey');if(anon)anon.value=elevenConfig.anonKey;
  refreshProviderForm();
  const auto=document.querySelector('#elevenAutoGenerate');if(auto)auto.checked=elevenConfig.autoGenerate;
}

let adminClickTimes=[],adminHoldTimer=null,adminModuleKey='',adminSceneIndex=0;
...
const DEPLOYMENT_CONFIG=window.KC_DEPLOYMENT_CONFIG||{};
const DEFAULT_SUPABASE_URL=DEPLOYMENT_CONFIG.supabase?.projectUrl||'https://iddudrxuihdodnvejxcp.supabase.co';
const DEFAULT_SUPABASE_KEY=DEPLOYMENT_CONFIG.supabase?.publishableKey||'';
function setCloudCheck(key,state,label=''){const el=document.querySelector(`[data-check="${key}"]`);if(!el)return;el.className=state==='ok'?'check-ok':state==='error'?'check-error':state==='working'?'check-working':'';const icon=state==='ok'?'✓':state==='error'?'✕':state==='working'?'…':'○';el.textContent=`${icon} ${label||el.textContent.replace(/^[✓✕…○]\s*/,'')}`}
function refreshSupabaseControls(){const url=document.querySelector('#supabaseProjectUrl'),key=document.querySelector('#supabasePublishableKey'),save=document.querySelector('#supabaseSave'),test=document.querySelector('#supabaseTest'),sync=document.querySelector('#supabaseSyncNow');if(url&&!url.value.trim())url.value=DEFAULT_SUPABASE_URL;const hasUrl=Boolean(url?.value.trim()),hasKey=Boolean(key?.value.trim());setCloudCheck('url',hasUrl?'ok':'error','Projekt-URL');setCloudCheck('key',hasKey?'ok':'','Publishable Key');if(save)save.disabled=!(hasUrl&&hasKey);if(test)test.disabled=!(hasUrl&&hasKey);const d=window.KCSupabaseAdapter?.diagnostics?.()||{};if(sync)sync.disabled=!(d.authenticated&&d.lastResult?.connectionTest===true);}
function supabaseConfigToForm(){const c=window.KCSupabaseAdapter?.config||{};const q=id=>document.querySelector(id);if(q('#supabaseProjectUrl'))q('#supabaseProjectUrl').value=c.projectUrl||DEFAULT_SUPABASE_URL;if(q('#supabasePublishableKey'))q('#supabasePublishableKey').value=c.publishableKey||DEFAULT_SUPABASE_KEY||'';if(q('#supabaseEnabled'))q('#supabaseEnabled').checked=Boolean(c.enabled);if(q('#supabaseAutoSync'))q('#supabaseAutoSync').checked=c.autoSync!==false;if(q('#supabaseOfflineAllowed'))q('#supabaseOfflineAllowed').checked=c.offlineAllowed!==false;if(q('#supabaseSyncInterval'))q('#supabaseSyncInterval').value=String(c.syncIntervalSeconds||60);q('#supabasePublishableKey')?.addEventListener('input',refreshSupabaseControls);q('#supabaseProjectUrl')?.addEventListener('input',refreshSupabaseControls);const toggle=q('#supabaseKeyToggle');if(toggle)toggle.onclick=()=>{const i=q('#supabasePublishableKey');i.type=i.type==='password'?'text':'password';toggle.textContent=i.type==='password'?'👁':'🙈'};refreshSupabaseControls()}
function supabaseMessage(text,kind=''){const e=document.querySelector('#supabaseAdminMessage');if(!e)return;e.textContent=text;e.className='settings-note '+(kind?`sync-status-${kind}`:'')}
function readSupabaseForm(){return {projectUrl:document.querySelector('#supabaseProjectUrl')?.value.trim()||DEFAULT_SUPABASE_URL,publishableKey:document.querySelector('#supabasePublishableKey')?.value.trim()||DEFAULT_SUPABASE_KEY||'',enabled:Boolean(document.querySelector('#supabaseEnabled')?.checked),autoSync:Boolean(document.querySelector('#supabaseAutoSync')?.checked),offlineAllowed:Boolean(document.querySelector('#supabaseOfflineAllowed')?.checked),syncIntervalSeconds:Number(document.querySelector('#supabaseSyncInterval')?.value)||60}}
function showSupabaseTestDetails(r){const host=document.querySelector('#supabaseTestDetails');if(!host)return;host.hidden=false;const step=(ok,label,detail='')=>`<div class="cloud-test-row ${ok?'ok':'fail'}"><b>${ok?'✓':'✕'} ${esc(label)}</b><span>${esc(String(detail||''))}</span></div>`;host.innerHTML=[step(r.authOk,'Anonyme Anmeldung',r.userId?`Benutzer-ID ${r.userId}`:'nicht bestätigt'),step(r.dbOk,'Datenbank erreichbar',r.table||''),step(r.rlsOk,'RLS-Zugriff',r.rlsMessage||''),step(r.writeOk,'Testdatensatz schreiben',r.writeMessage||''),step(r.readOk,'Testdatensatz lesen',r.readMessage||''),step(r.cleanupOk,'Testdatensatz bereinigen',r.cleanupMessage||''),step(r.ok,'Synchronisation bereit',r.latencyMs!=null?`${r.latencyMs} ms`:'')].join('')}
async function saveSupabaseForm(){window.KCSupabaseAdapter?.configure(readSupabaseForm());updateDbStatus();supabaseMessage('Verbindung verschlüsselt gespeichert. Jetzt bitte testen.','ok');refreshSupabaseControls();renderAdminSystem()}
async function testSupabaseConnection(){supabaseMessage('Verbindung, Anmeldung, RLS sowie Schreiben und Lesen werden geprüft …','warn');['auth','db','rls','sync'].forEach(k=>setCloudCheck(k,'working'));try{window.KCSupabaseAdapter?.configure({...readSupabaseForm(),enabled:true});const r=await window.KCSupabaseAdapter.testConnection({full:true});showSupabaseTestDetails(r);setCloudCheck('auth',r.authOk?'ok':'error','Anonyme Anmeldung');setCloudCheck('db',r.dbOk?'ok':'error','Datenbank');setCloudCheck('rls',r.rlsOk?'ok':'error','RLS');setCloudCheck('sync',r.ok?'ok':'error','Synchronisation');if(!r.ok)throw new Error(r.message||'Mindestens ein Prüfschritt ist fehlgeschlagen.');document.querySelector('#supabaseEnabled').checked=true;window.KCSupabaseAdapter.configure(readSupabaseForm());supabaseMessage(`Verbindung hergestellt. Future Academy ist online bereit (${r.latencyMs} ms).`,'ok');updateDbStatus();refreshSupabaseControls();renderAdminSystem();renderPerformanceGauges();renderCloudDeviceMonitor()}catch(e){supabaseMessage('Verbindung fehlgeschlagen: '+e.message,'error');updateDbStatus();refreshSupabaseControls()}}
async function syncSupabaseNow(){supabaseMessage('Synchronisation läuft …','warn');try{window.KCSupabaseAdapter?.configure({...readSupabaseForm(),enabled:true});const r=await window.KCSupabaseAdapter.sync();if(!r.ok&&r.reason)throw new Error(r.message||r.reason);supabaseMessage(`Synchronisiert: ${r.sent||0} übertragen, ${r.pending||0} ausstehend.`,'ok');setCloudCheck('sync','ok','Synchronisation');updateDbStatus();renderAdminSystem();renderPerformanceGauges();renderCloudDeviceMonitor()}catch(e){supabaseMessage('Synchronisation fehlgeschlagen: '+e.message,'error');setCloudCheck('sync','error','Synchronisation');updateDbStatus()}}

function gaugeMeta(kind,value){
  const n=Math.max(0,Number(value)||0);
  if(kind==='local'){const score=Math.min(1,n/250);return {deg:Math.round(score*270),status:n>=140?'sehr schnell':n>=70?'gut':'langsam',cls:n>=140?'speed-status-fast':n>=70?'speed-status-medium':'speed-status-slow'}}
  const score=Math.max(0,Math.min(1,1-(n-80)/1120));return {deg:Math.round(score*270),status:n<=350?'sehr gut':n<=750?'gut':'langsam',cls:n<=350?'speed-status-fast':n<=750?'speed-status-medium':'speed-status-slow'}
}
function speedGaugeCard({kind,title,value,unit,statusDetail,rows=[]}){const m=gaugeMeta(kind,value);return `<article class="speed-gauge-card"><div class="speed-gauge" style="--gauge:${m.deg}deg" role="img" aria-label="${esc(title)} ${esc(String(value))} ${esc(unit)}"><div class="speed-gauge-value"><b>${esc(String(value))}</b><small>${esc(unit)}</small></div></div><div class="speed-gauge-copy"><h4>${esc(title)}</h4><p class="${m.cls}">${esc(m.status)}</p><p>${esc(statusDetail||'')}</p><dl>${rows.map(([a,b])=>`<dt>${esc(a)}</dt><dd>${esc(String(b))}</dd>`).join('')}</dl></div></article>`}
async function renderPerformanceGauges(){const host=document.querySelector('#performanceGauges');if(!host)return;host.innerHTML='<p class="settings-note">Geschwindigkeit wird gemessen …</p>';let local={opsPerSecond:0,writeMs:'—',readMs:'—',totalMs:'—'};try{await window.KCSecureStorage?.ready;local=await window.KCSecureStorage?.benchmark?.()||local}catch(e){console.warn('IndexedDB benchmark failed',e)}const d=window.KCSupabaseAdapter?.diagnostics?.()||{};const lr=d.lastResult||{};const latency=Number(lr.latencyMs||lr.durationMs||0);const cloudValue=latency||0;host.innerHTML=speedGaugeCard({kind:'local',title:'IndexedDB',value:local.opsPerSecond||0,unit:'Vorgänge/s',statusDetail:'Lokaler verschlüsselter Speicher',rows:[['Schreiben',`${local.writeMs} ms`],['Lesen',`${local.readMs} ms`],['Gesamt',`${local.totalMs} ms`]]})+speedGaugeCard({kind:'cloud',title:'Supabase Cloud',value:cloudValue||'—',unit:cloudValue?'ms':'kein Messwert',statusDetail:d.authenticated?'Online und angemeldet':'Noch kein aktueller Verbindungstest',rows:[['Letzte Übertragung',lr.sent??0],['Datensätze/s',lr.recordsPerSecond??'—'],['Ausstehend',lr.pending??0],['Letzte Aktivität',lr.at?new Date(lr.at).toLocaleString('de-DE'):'—']]})}
function deviceIcon(name=''){const n=String(name).toLowerCase();if(n.includes('ipad')||n.includes('iphone'))return '📱';if(n.includes('android'))return '📱';if(n.includes('windows')||n.includes('pc')||n.includes('mac'))return '💻';return '🖥️'}
async function renderCloudDeviceMonitor(){const list=document.querySelector('#cloudDeviceList'),counters=document.querySelector('#cloudSyncCounters'),nameInput=document.querySelector('#cloudDeviceName');if(!list||!counters)return;const a=window.KCSupabaseAdapter,d=a?.diagnostics?.()||{},st=d.stats||{},lr=d.lastResult||{};if(nameInput&&!nameInput.value)nameInput.value=d.deviceName||'';counters.innerHTML=`<article class="cloud-counter"><b>${Number(st.uploaded||0)}</b><span>Hochgeladen</span></article><article class="cloud-counter"><b>${Number(st.downloaded||0)}</b><span>Heruntergeladen</span></article><article class="cloud-counter"><b>${Number(lr.pending||0)}</b><span>Warteschlange</span></article><article class="cloud-counter"><b>${st.lastSync?esc(new Date(st.lastSync).toLocaleTimeString('de-DE')):'—'}</b><span>Letzte Synchronisation</span></article>`;list.innerHTML='<p class="settings-note">Geräte werden geladen …</p>';let devices=[];try{if(d.authenticated)devices=await a.listDevices()}catch(e){console.warn('Device list failed',e)}if(!devices.length){devices=[{installation_id:d.installationId,client_created_at:st.lastHeartbeat||new Date().toISOString(),academy_version:ACADEMY_VERSION,metadata:{device_name:d.deviceName||'Dieses Gerät',device_type:d.deviceName||'',online:d.online}}]}list.innerHTML=devices.map(row=>{const m=row.metadata||{},seen=row.client_created_at||m.last_seen||'',age=seen?Date.now()-new Date(seen).getTime():Infinity,online=age<120000&&d.online;return `<article class="cloud-device-card"><span class="cloud-device-icon">${deviceIcon(m.device_type||m.device_name)}</span><div class="cloud-device-copy"><b>${esc(m.device_name||'Unbenanntes Gerät')}</b><small>${esc(row.academy_version||'')} · ${seen?`zuletzt ${esc(new Date(seen).toLocaleString('de-DE'))}`:'kein Kontakt'}</small><small>${esc(row.installation_id||'')}</small></div><span class="cloud-device-state ${online?'':'offline'}">${online?'Online':'Zuletzt gesehen'}</span></article>`}).join('')}
function renderAdminSystem(){const host=document.querySelector('#adminSystemStatus');if(!host)return;const d=window.KCSupabaseAdapter?.diagnostics?.()||{},lr=d.lastResult||{};host.innerHTML=`<article><b>IndexedDB</b><span class="status-ok">verbunden</span><small>Verschlüsselter Secure Storage aktiv</small></article><article><b>Supabase</b><span class="${d.enabled&&d.configured&&d.authenticated?'status-ok':'status-warn'}">${d.enabled&&d.configured?(d.online?(d.authenticated?'verbunden':'bereit'):'offline'):'noch nicht eingerichtet'}</span><small>${d.authenticated?'Anonyme Sitzung aktiv':d.configured?'Anmeldung erfolgt beim ersten Test':'Projekt-URL und Publishable Key erforderlich'}</small></article><article><b>Synchronisation</b><span>${lr.at?esc(new Date(lr.at).toLocaleString('de-DE')):'noch nicht ausgeführt'}</span><small>${lr.sent||0} zuletzt übertragen · ${lr.pending||0} ausstehend</small></article><article><b>Cloud-Benutzer</b><span>${d.userId?esc(d.userId):'—'}</span><small>Installation ${esc(d.installationId||'—')}</small></article><article><b>Voice Provider</b><span>${esc(state.voiceProvider)}</span></article><article><b>Version</b><span>${ACADEMY_VERSION}</span></article>`;supabaseConfigToForm();const b=id=>document.querySelector(id);if(b('#supabaseSave'))b('#supabaseSave').onclick=saveSupabaseForm;if(b('#supabaseTest'))b('#supabaseTest').onclick=testSupabaseConnection;if(b('#supabaseSyncNow'))b('#supabaseSyncNow').onclick=syncSupabaseNow;if(b('#performanceRefresh'))b('#performanceRefresh').onclick=renderPerformanceGauges;if(b('#cloudDeviceRefresh'))b('#cloudDeviceRefresh').onclick=()=>{window.KCSupabaseAdapter?.registerDevice?.().catch(()=>{});renderCloudDeviceMonitor()};if(b('#cloudDeviceSave'))b('#cloudDeviceSave').onclick=async()=>{const n=b('#cloudDeviceName')?.value||'';window.KCSupabaseAdapter?.setDeviceName?.(n);await window.KCSupabaseAdapter?.registerDevice?.().catch(()=>{});renderCloudDeviceMonitor()};renderPerformanceGauges();renderCloudDeviceMonitor()}
function renderDiagnostics(){const host=document.querySelector('#diagnosticsContent');if(!host)return;const d=window.KCSupabaseAdapter?.diagnostics?.()||{};host.innerHTML=`<article><b>Lokale Daten</b><span class="status-ok">bereit</span><small>Verschlüsselter Speicher aktiv</small></article><article><b>Online-Verbindung</b><span class="${d.enabled&&d.configured&&d.online?'status-ok':'status-warn'}">${d.enabled&&d.configured?(d.online?'bereit':'offline'):'nicht eingerichtet'}</span><small>${d.authenticated?'Automatische Anmeldung aktiv':d.configured?'Anmeldung wird automatisch versucht':'Supportbericht kann trotzdem erstellt werden'}</small></article><article><b>Ton</b><span>${state.sound?'aktiv':'aus'}</span><small>Lautstärke ${state.volume}/10</small></article><article><b>Version</b><span>${ACADEMY_VERSION}</span><small>KC FUTURA Academy</small></article>`}
function openDiagnostics(){renderDiagnostics();document.querySelector('#diagnosticsModal').hidden=false;document.body.classList.add('modal-open')}
...
function reportActiveCount(){return Object.values(adminReportFilters).filter(v=>v&&v!=='all').length}
function localLearningRows(){const sum=TelemetryCore.summary(),now=new Date().toISOString(),device=window.KCSupabaseAdapter?.diagnostics?.().deviceName||'Dieses Gerät';const rows=[{participant_id:participantProfile?.id||'local',participant_name:state.name||participantProfile?.displayName||'Unbekannt',device_name:device,last_activity:sum.rows.at(-1)?.at||now,total_minutes:sum.totalMinutes,total_correct:sum.totalCorrect,total_wrong:sum.totalWrong,completed_modules:Object.keys(state.completed).length,total_runs:sum.starts.length,topics:Object.entries(sum.byModule).map(([id,v])=>({id,title:modules[id]?.title||id,...v})),source:'academy'}];const training=window.KCParticipantDataCore?.trainingToRow?.(JSON.parse(localStorage.getItem('kc_training_profile_v0254')||'null'),participantProfile,device);if(training&&training.participant_name!=='Unbekannt'){const i=rows.findIndex(r=>r.participant_id===training.participant_id||r.participant_name===training.participant_name);if(i>=0){rows[i]={...rows[i],total_minutes:Number(rows[i].total_minutes||0)+Number(training.total_minutes||0),total_correct:Number(rows[i].total_correct||0)+Number(training.total_correct||0),total_wrong:Number(rows[i].total_wrong||0)+Number(training.total_wrong||0),completed_modules:Number(rows[i].completed_modules||0)+Number(training.completed_modules||0),total_runs:Number(rows[i].total_runs||0)+Number(training.total_runs||0),last_activity:[rows[i].last_activity,training.last_activity].sort().at(-1),topics:[...(rows[i].topics||[]),...(training.topics||[])],source:'combined'}}else rows.push(training)}return rows}
function periodMatches(date,period){if(period==='all')return true;const age=Date.now()-new Date(date||0).getTime(),day=86400000;if(period==='today')return age<day;if(period==='week')return age<7*day;if(period==='month')return age<31*day;return true}
async function fetchAdminLearningRows(){await window.KCParticipantDataCore?.migrate?.().catch(()=>{});const local=localLearningRows();try{const cloud=await window.KCSupabaseAdapter?.adminLearningReport?.();if(Array.isArray(cloud)&&cloud.length){const map=new Map(local.map(r=>[r.participant_id||r.participant_name,r]));cloud.forEach(r=>map.set(r.participant_id||r.participant_name,{...(map.get(r.participant_id||r.participant_name)||{}),...r,source:'cloud'}));return [...map.values()]}}catch(e){console.warn('Zentrale Adminauswertung nicht verfügbar',e)}return local}
async function renderAdminLearningReport(){const host=document.querySelector('#adminLearningReport');if(!host)return;host.innerHTML='<p class="settings-note">Auswertung wird geladen …</p>';const rows=await fetchAdminLearningRows();const persons=[...new Set(rows.map(r=>r.participant_name).filter(Boolean))].sort(),devices=[...new Set(rows.map(r=>r.device_name).filter(Boolean))].sort();const filtered=rows.filter(r=>(adminReportFilters.person==='all'||r.participant_name===adminReportFilters.person)&&(adminReportFilters.device==='all'||r.device_name===adminReportFilters.device)&&periodMatches(r.last_activity,adminReportFilters.period)&&(adminReportFilters.status==='all'||(adminReportFilters.status==='active'?Number(r.total_minutes)>0:Number(r.completed_modules)>0)));const total=filtered.reduce((a,r)=>({minutes:a.minutes+Number(r.total_minutes||0),correct:a.correct+Number(r.total_correct||0),wrong:a.wrong+Number(r.total_wrong||0),runs:a.runs+Number(r.total_runs||0),completed:a.completed+Number(r.completed_modules||0)}),{minutes:0,correct:0,wrong:0,runs:0,completed:0});const active=reportActiveCount();host.innerHTML=`<div class="admin-report-head"><div><h3>Teilnehmer & Lernstatistik</h3><p class="settings-note">Wer hat wann, wie lange und mit welchem Ergebnis gelernt?</p></div><div class="actions"><button id="participantDataCheck" class="secondary">Teilnehmerdaten prüfen</button><button id="adminReportRefresh" class="secondary">Aktualisieren</button></div></div><div class="admin-report-filters"><label class="${adminReportFilters.person!=='all'?'filter-active':''}">Person<select data-report-filter="person"><option value="all">Alle Personen</option>${persons.map(x=>`<option ${adminReportFilters.person===x?'selected':''}>${esc(x)}</option>`).join('')}</select></label><label class="${adminReportFilters.period!=='all'?'filter-active':''}">Zeitraum<select data-report-filter="period"><option value="all">Gesamter Zeitraum</option><option value="today" ${adminReportFilters.period==='today'?'selected':''}>Heute</option><option value="week" ${adminReportFilters.period==='week'?'selected':''}>7 Tage</option><option value="month" ${adminReportFilters.period==='month'?'selected':''}>30 Tage</option></select></label><label class="${adminReportFilters.category!=='all'?'filter-active':''}">Thema<select data-report-filter="category"><option value="all">Alle Themen</option>${Object.entries(CATEGORY_DEFS).map(([k,v])=>`<option value="${k}" ${adminReportFilters.category===k?'selected':''}>${esc(v.label)}</option>`).join('')}</select></label><label class="${adminReportFilters.status!=='all'?'filter-active':''}">Status<select data-report-filter="status"><option value="all">Alle</option><option value="active" ${adminReportFilters.status==='active'?'selected':''}>Hat gelernt</option><option value="completed" ${adminReportFilters.status==='completed'?'selected':''}>Folgen abgeschlossen</option></select></label><label class="${adminReportFilters.device!=='all'?'filter-active':''}">Gerät<select data-report-filter="device"><option value="all">Alle Geräte</option>${devices.map(x=>`<option ${adminReportFilters.device===x?'selected':''}>${esc(x)}</option>`).join('')}</select></label><button id="adminReportReset" class="filter-toggle ${active?'has-active':''}">Filter zurücksetzen <span>${active} / 5</span></button></div><div class="admin-report-kpis"><article><b>${filtered.length}</b><span>Teilnehmer</span></article><article><b>${Math.round(total.minutes*10)/10} min</b><span>Lernzeit</span></article><article><b>${total.runs}</b><span>Durchgänge</span></article><article><b>${total.correct}</b><span>Richtig</span></article><article><b>${total.wrong}</b><span>Falsch</span></article><article><b>${total.completed}</b><span>Folgen abgeschlossen</span></article></div><div class="admin-report-table-wrap"><table class="admin-report-table"><thead><tr><th>Teilnehmer</th><th>Letzte Aktivität</th><th>Lernzeit</th><th>Durchgänge</th><th>Richtig</th><th>Falsch</th><th>Folgen</th><th>Gerät</th></tr></thead><tbody>${filtered.map(r=>`<tr><td><b>${esc(r.participant_name||'—')}</b><small>${esc(String(r.participant_id||'').slice(0,12))}</small></td><td>${r.last_activity?esc(new Date(r.last_activity).toLocaleString('de-DE')):'—'}</td><td>${Number(r.total_minutes||0).toFixed(1)} min</td><td>${Number(r.total_runs||0)}</td><td>${Number(r.total_correct||0)}</td><td>${Number(r.total_wrong||0)}</td><td>${Number(r.completed_modules||0)}</td><td>${esc(r.device_name||'—')}</td></tr>`).join('')||'<tr><td colspan="8">Keine Daten für diese Filter.</td></tr>'}</tbody></table></div><p class="settings-note">Zentrale Daten aller Teilnehmer erscheinen hier, sobald das Supabase-Superadmin-Konto eingerichtet ist. Bis dahin wird die aktuelle lokale Sitzung angezeigt.</p>`;host.querySelectorAll('[data-report-filter]').forEach(el=>el.onchange=()=>{adminReportFilters[el.dataset.reportFilter]=el.value;set('localStorage',ADMIN_REPORT_FILTERS_KEY,JSON.stringify(adminReportFilters));renderAdminLearningReport()});document.querySelector('#adminReportReset').onclick=()=>{adminReportFilters={person:'all',period:'all',category:'all',status:'all',device:'all'};set('localStorage',ADMIN_REPORT_FILTERS_KEY,JSON.stringify(adminReportFilters));renderAdminLearningReport()};document.querySelector('#adminReportRefresh').onclick=renderAdminLearningReport;document.querySelector('#participantDataCheck').onclick=async()=>{const d=await window.KCParticipantDataCore?.diagnostics?.();alert(`Teilnehmerdaten\nName: ${d?.name||'nicht gefunden'}\nID: ${d?.participantId||'-'}\nLokales Profil: ${d?.profileFound?'JA':'NEIN'}\nHauptschulung: ${d?.trainingFound?'JA':'NEIN'}\nSicherer Speicher: ${d?.secureStorage?'JA':'NEIN'}\nLetzte Aktivität: ${d?.lastActivity||'-'}`)}}
function bindSecretAccess(){const logo=document.querySelector('.brand-logo');if(!logo)return;logo.tabIndex=0;logo.title='';logo.addEventListener('click',()=>{const now=Date.now();adminClickTimes=adminClickTimes.filter(t=>now-t<2500);adminClickTimes.push(now);if(adminClickTimes.length>=7){adminClickTimes=[];openAdminPin()}});logo.addEventListener('pointerdown',()=>{clearTimeout(adminHoldTimer);adminHoldTimer=setTimeout(openAdminPin,5000)});['pointerup','pointerleave','pointercancel'].forEach(e=>logo.addEventListener(e,()=>clearTimeout(adminHoldTimer)))}
function openSettings(){stopSpeech();settingsModal.hidden=false;document.body.classList.add('modal-open');document.querySelector('#modalCoach').value=state.coach;document.querySelector('#modalAddress').value=state.address;document.querySelector('#modalRate').value=state.speechRate;document.querySelector('#modalSubtitles').checked=state.subtitles;document.querySelector('#modalSound').checked=state.sound;document.querySelector('#modalQuizMode').value=state.quizMode;document.querySelector('#modalEpisodeView').value=state.episodeView;document.querySelector('#modalRememberView').checked=state.rememberView;document.querySelector('#modalShuffleAnswers').checked=state.shuffleAnswers;document.querySelector('#modalCountdown').checked=state.countdownEnabled;document.querySelector('#modalAutoContinue').checked=state.autoContinue;document.querySelector('#modalSkipIntroductions').checked=state.skipIntroductions;document.querySelector('#modalOpenLast').checked=state.openLast;document.querySelector('#modalVolume').value=state.volume;document.querySelector('#modalVolumeValue').textContent=`${state.volume}/10`;document.querySelector('#voiceProvider').value=state.voiceProvider;window.KCVoiceProvider?.configure({...elevenConfig,provider:state.voiceProvider});document.querySelector('#designFontScale').value=state.fontScale;document.querySelector('#designReduceMotion').checked=state.reduceMotion;refreshVoiceStudio();refreshCharacterStudio()}
function closeSettings(){settingsModal.hidden=true;document.body.classList.remove('modal-open');stopRecorder()}
function stopRecorder(){if(recorderState.mediaRecorder&&recorderState.mediaRecorder.state==='recording')recorderState.mediaRecorder.stop();if(recorderState.stream){recorderState.stream.getTracks().forEach(t=>t.stop());recorderState.stream=null}}
async function startRecording(){try{if(!navigator.mediaDevices?.getUserMedia||!window.MediaRecorder)throw new Error('Audioaufnahme wird von diesem Browser nicht unterstützt');resetRecorderPreview();const stream=await navigator.mediaDevices.getUserMedia({audio:{echoCancellation:true,noiseSuppression:true,autoGainControl:true}});recorderState.stream=stream;recorderState.chunks=[];const preferred=['audio/webm;codecs=opus','audio/webm','audio/mp4'].find(t=>MediaRecorder.isTypeSupported?.(t));const mr=new MediaRecorder(stream,preferred?{mimeType:preferred}:undefined);recorderState.mediaRecorder=mr;mr.ondataavailable=e=>{if(e.data.size)recorderState.chunks.push(e.data)};mr.onstop=()=>{recorderState.blob=new Blob(recorderState.chunks,{type:mr.mimeType||'audio/webm'});recorderState.url=URL.createObjectURL(recorderState.blob);document.querySelector('#recordPlay').disabled=false;document.querySelector('#recordSave').disabled=false;document.querySelector('#recordingStatus').textContent='Aufnahme bereit – bitte anhören und speichern';if(recorderState.stream){recorderState.stream.getTracks().forEach(t=>t.stop());recorderState.stream=null}};mr.start();document.querySelector('#recordStart').disabled=true;document.querySelector('#recordStop').disabled=false;document.querySelector('#recordingStatus').textContent='● Aufnahme läuft'}catch(err){document.querySelector('#recordingStatus').textContent='Mikrofonzugriff nicht möglich: '+(err?.message||'unbekannter Fehler')}}
function stopRecording(){if(recorderState.mediaRecorder?.state==='recording')recorderState.mediaRecorder.stop();document.querySelector('#recordStart').disabled=false;document.querySelector('#recordStop').disabled=true}
function playRecording(){if(recorderState.url)new Audio(recorderState.url).play()}
async function saveRecording(){if(!recorderState.blob||!recorderState.dialogKey)return;await AudioStore.put(recorderState.dialogKey,recorderState.blob,{character:document.querySelector('#voiceCharacter').value,text:document.querySelector('#voiceText').value});document.querySelector('#recordingStatus').textContent='Aufnahme gespeichert';document.querySelector('#recordDelete').disabled=false}
async function deleteRecording(){if(!recorderState.dialogKey)return;await AudioStore.remove(recorderState.dialogKey);resetRecorderPreview();document.querySelector('#recordDelete').disabled=true;document.querySelector('#recordingStatus').textContent='Aufnahme gelöscht – Gerätestimme wird verwendet'}
function importAudio(file){if(!file||!recorderState.dialogKey)return;resetRecorderPreview();recorderState.blob=file;recorderState.url=URL.createObjectURL(file);document.querySelector('#recordPlay').disabled=false;document.querySelector('#recordSave').disabled=false;document.querySelector('#recordingStatus').textContent='Audiodatei geladen – bitte anhören und speichern'}
function providerDefinition(provider=state.voiceProvider){const defs={elevenlabs:{label:'ElevenLabs',models:[['eleven_multilingual_v2','Multilingual v2'],['eleven_flash_v2_5','Flash v2.5'],['eleven_v3','Eleven v3']],formats:[['mp3_44100_128','MP3 44,1 kHz / 128 kbit']],voicePlaceholder:'Voice-ID aus ElevenLabs',secret:'ELEVENLABS_API_KEY'},openai:{label:'OpenAI TTS',models:[['gpt-4o-mini-tts','GPT-4o mini TTS']],formats:[['mp3','MP3'],['wav','WAV'],['opus','Opus'],['aac','AAC'],['flac','FLAC']],voicePlaceholder:'z. B. coral',secret:'OPENAI_API_KEY'},azure:{label:'Microsoft Azure Speech',models:[['azure-neural','Azure Neural Voice']],formats:[['audio-24khz-48kbitrate-mono-mp3','MP3 24 kHz'],['riff-24khz-16bit-mono-pcm','WAV 24 kHz']],voicePlaceholder:'z. B. de-DE-KatjaNeural',secret:'AZURE_SPEECH_KEY + AZURE_SPEECH_REGION'}};return defs[provider]||defs.elevenlabs}
function refreshProviderForm(){const p=state.voiceProvider,d=providerDefinition(p),s=elevenConfig.providerSettings?.[p]||{};const m=document.querySelector('#elevenModel');if(m){m.innerHTML=d.models.map(x=>`<option value="${x[0]}">${x[1]}</option>`).join('');m.value=s.modelId||d.models[0][0]}const f=document.querySelector('#providerOutputFormat');if(f){f.innerHTML=d.formats.map(x=>`<option value="${x[0]}">${x[1]}</option>`).join('');f.value=s.outputFormat||d.formats[0][0]}const i=document.querySelector('#providerInstructions');if(i)i.value=s.instructions||'';const v=document.querySelector('#elevenVoiceId');if(v)v.placeholder=d.voicePlaceholder;const sum=document.querySelector('#providerSummary');if(sum)sum.innerHTML=`<b>${d.label}</b><span>Supabase Secret: <code>${d.secret}</code></span><span>Es fehlen nur Supabase-URL, Publishable Key und die feste Stimme je Figur.</span>`;const il=document.querySelector('#providerInstructionsLabel');if(il)il.hidden=p!=='openai'}
function readElevenForm(){elevenConfig.endpoint=document.querySelector('#elevenEndpoint')?.value.trim()||'';elevenConfig.anonKey=document.querySelector('#elevenAnonKey')?.value.trim()||'';const d=providerDefinition(state.voiceProvider),s=elevenConfig.providerSettings[state.voiceProvider]||{};s.modelId=document.querySelector('#elevenModel')?.value||d.models[0][0];s.outputFormat=document.querySelector('#providerOutputFormat')?.value||d.formats[0][0];s.instructions=document.querySelector('#providerInstructions')?.value.trim()||'';elevenConfig.providerSettings[state.voiceProvider]=s;elevenConfig.modelId=s.modelId;elevenConfig.outputFormat=s.outputFormat;elevenConfig.autoGenerate=Boolean(document.querySelector('#elevenAutoGenerate')?.checked);elevenConfig.voiceIds[state.voiceProvider]=elevenConfig.voiceIds[state.voiceProvider]||{};const c=document.querySelector('#voiceCharacter')?.value;if(c)elevenConfig.voiceIds[state.voiceProvider][c]=document.querySelector('#elevenVoiceId')?.value.trim()||'';saveElevenConfig()}
function currentVoiceDialog(){return buildDialogRegistry().find(x=>x.key===document.querySelector('#voiceDialog')?.value)}
function setElevenStatus(message,kind=''){const el=document.querySelector('#elevenStatus');if(!el)return;el.textContent=message;el.className='settings-note'+(kind?` ${kind}`:'')}
async function saveElevenForm(){readElevenForm();setElevenStatus('Provider-Verbindung und feste Stimmenzuordnung wurden lokal gespeichert.','tuv-pass');await refreshVoiceStudio()}
async function testElevenConnection(){
  try{readElevenForm();setElevenStatus('Test läuft …');const dialog=currentVoiceDialog();if(!dialog)throw new Error('Bitte zuerst einen Dialog auswählen.');await generateElevenAudio({...dialog,text:`Dies ist ein Test von ${providerDefinition(state.voiceProvider).label} für die KC FUTURA Stimmen.`},{previewOnly:true});setElevenStatus('Verbindung erfolgreich. Die Teststimme wurde gestartet.','tuv-pass')}
  catch(err){setElevenStatus('Test fehlgeschlagen: '+(err?.message||err),'tuv-fail')}
}
async function generateCurrentVoice(){
  const status=document.querySelector('#recordingStatus');try{readElevenForm();const dialog=currentVoiceDialog();if(!dialog)throw new Error('Kein Dialog ausgewählt.');status.textContent='Stimme wird erzeugt …';status.className='recording-status voice-working';await generateElevenAudio(dialog);setElevenStatus('Sprachdatei erzeugt und verschlüsselt gespeichert.','tuv-pass');await refreshVoiceStudio()}
  catch(err){status.textContent='Erzeugung fehlgeschlagen: '+(err?.message||err);status.className='recording-status voice-stale'}
}
async function generateAllStaleVoices(){
  try{readElevenForm();const registry=buildDialogRegistry();let stale=0,done=0,skipped=0;for(const dialog of registry){const a=await voiceAudioState(dialog);if(a.kind!=='stale')continue;stale++;if(!voiceIdFor(dialog.character)){skipped++;continue}setElevenStatus(`Aktualisierung ${done+1} von ${stale} …`);await generateElevenAudio(dialog);done++}setElevenStatus(`${done} veraltete Stimmen aktualisiert${skipped?`, ${skipped} ohne Voice-ID übersprungen`:''}.`,'tuv-pass');await refreshVoiceStudio()}
  catch(err){setElevenStatus('Sammelaktualisierung abgebrochen: '+(err?.message||err),'tuv-fail')}
}

function openExit(){stopSpeech();exitModal.hidden=false;document.body.classList.add('modal-open')}
function closeExit(){exitModal.hidden=true;document.body.classList.remove('modal-open')}
function confirmExit(){closeExit();stopSpeech();state.paused=false;pauseBtn.textContent='⏸ Pause';try{location.href=returnUrl}catch{welcome()}}
function applyTheme(theme){if(!['classic','winter','contrast'].includes(theme))theme='classic';state.theme=theme;document.body.dataset.theme=theme;document.documentElement.style.fontSize=`${state.fontScale*100}%`;document.body.classList.toggle('reduce-motion',state.reduceMotion);saveSettings();document.querySelectorAll('.theme-preview').forEach(b=>b.classList.toggle('selected',b.dataset.theme===theme))}


let characterStudioKey='laura';
function refreshCharacterStudio(){
  const list=document.querySelector('#characterStudioList');if(!list)return;
  list.innerHTML=Object.entries(chars).map(([key,c])=>`<button class="character-studio-item ${key===characterStudioKey?'selected':''}" data-character-edit="${key}"><img src="${AV+c.img}" alt="${c.name}"><span><b>${esc(c.name)}</b><small>${esc(c.role)}</small></span></button>`).join('');
  list.querySelectorAll('[data-character-edit]').forEach(b=>b.onclick=()=>{characterStudioKey=b.dataset.characterEdit;refreshCharacterStudio()});
  const c=chars[characterStudioKey];document.querySelector('#characterStudioImage').src=AV+c.img;document.querySelector('#characterStudioImage').alt=c.name;document.querySelector('#characterStudioName').value=c.name;document.querySelector('#characterStudioRole').value=c.role;document.querySelector('#characterStudioIntro').value=c.intro;
}
function saveCharacterProfile(){const c=chars[characterStudioKey];if(!c)return;c.name=document.querySelector('#characterStudioName').value.trim()||c.name;c.role=document.querySelector('#characterStudioRole').value.trim()||c.role;c.intro=document.querySelector('#characterStudioIntro').value.trim()||c.intro;const all=safeParse(get('localStorage',CHARACTER_OVERRIDE_KEY,'{}'),{});all[characterStudioKey]={name:c.name,role:c.role,intro:c.intro};set('localStorage',CHARACTER_OVERRIDE_KEY,JSON.stringify(all));refreshCharacterStudio();TelemetryCore.event('character_profile_saved',{character:characterStudioKey});autoRegenerateDialog(`character:${characterStudioKey}:intro`)}
function resetCharacterProfile(){Object.assign(chars[characterStudioKey],JSON.parse(JSON.stringify(defaultChars[characterStudioKey])));const all=safeParse(get('localStorage',CHARACTER_OVERRIDE_KEY,'{}'),{});delete all[characterStudioKey];set('localStorage',CHARACTER_OVERRIDE_KEY,JSON.stringify(all));refreshCharacterStudio()}
function downloadJson(name,data){const blob=new Blob([JSON.stringify(data,null,2)],{type:'application/json'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=name;a.click();setTimeout(()=>URL.revokeObjectURL(url),500)}
function exportVoiceConfig(){downloadJson('kc-futura-voice-config.json',{version:VOICE_CORE_VERSION,provider:state.voiceProvider,providers:{...elevenConfig,anonKey:elevenConfig.anonKey?'[lokal gespeichert]':''},profiles:voiceProfiles,dialogs:buildDialogRegistry().map(x=>({key:x.key,character:x.character,label:x.label,text:x.text}))})}
function openRoadmap(){
  stopSpeech();roadmapModal.hidden=false;document.body.classList.add('modal-open');
  const catalog=SceneCore.catalog();const groups={fertig:[], 'in-arbeit':[], geplant:[], idee:[]};
  catalog.forEach(m=>{const key=m.status==='fertig'?'fertig':m.status==='in-arbeit'?'in-arbeit':m.status==='idee'?'idee':'geplant';groups[key].push(m)});
  const completed=catalog.filter(m=>state.completed[m.id]).length;
  const totalScenes=catalog.reduce((n,m)=>n+m.sceneCount,0);const totalQuestions=catalog.reduce((n,m)=>n+m.questionCount,0);
  const render=(title,key,cls)=>`<section class="roadmap-column"><h3>${title}</h3>${groups[key].map(m=>`<article class="roadmap-item ${cls}"><span class="episode-badge">Staffel ${m.season} · Folge ${m.episode}</span><b>${esc(m.title)}</b><small>${esc(m.subtitle)}</small><div class="metadata-strip"><span>${m.sceneCount} Szenen</span><span>${m.questionCount} Fragen</span><span>ca. ${m.estimatedMinutes} Min.</span></div></article>`).join('')||'<p class="settings-note">Keine Einträge.</p>'}</section>`;
  document.querySelector('#roadmapContent').innerHTML=`<div class="roadmap-stats"><div><b>${catalog.length}</b><span>Folgen geplant</span></div><div><b>${catalog.filter(m=>m.ready).length}</b><span>Spielbar</span></div><div><b>${totalScenes}</b><span>Szenen gebaut</span></div><div><b>${totalQuestions}</b><span>Wissensfragen</span></div></div><div class="roadmap-board">${render('Fertig','fertig','ready')}${render('In Arbeit','in-arbeit','progress')}${render('Geplant','geplant','planned')}${render('Ideenspeicher','idee','idea')}</div><div class="actions"><button id="sceneManifestExport" class="secondary">Szenen-Manifest exportieren</button></div>`;
  document.querySelector('#sceneManifestExport').onclick=()=>SceneCore.export();TelemetryCore.event('roadmap_opened');
}
function closeRoadmap(){roadmapModal.hidden=true;document.body.classList.remove('modal-open')}

const projectDocs=[
  {title:'Pflichtenheft V1.0',file:'docs/PFLICHTENHEFT_KC_FUTURA_ACADEMY_V1_0.md',desc:'Ziele, Module, Cores, Dashboard, Datenschutz und verbindliche Qualitätsanforderungen.'},
  {title:'Studio-Handbuch V1.0',file:'docs/STUDIO_HANDBUCH_KC_FUTURA_ACADEMY_V1_0.md',desc:'Architektur-, UI-, Tablet-, Voice-, Character- und Designregeln.'},
  {title:'TÜV-Handbuch V1.0',file:'docs/TUEV_HANDBUCH_KC_FUTURA_ACADEMY_V1_0.md',desc:'Prüfkatalog, Regression, Robustheit und Freigabekriterien.'},
  {title:'Roadmap 2026',file:'docs/ROADMAP_KC_FUTURA_ACADEMY_2026.md',desc:'Prioritäten, nächste Module, Episodenpool und Plattformausbau.'},
  {title:'Ideenkatalog',file:'docs/IDEENKATALOG_KC_FUTURA_ACADEMY.md',desc:'Figuren, Themenfelder, Leitmotive und Produktionsregeln.'}
];
function openDocs(){
  stopSpeech();docsModal.hidden=false;document.body.classList.add('modal-open');
  document.querySelector('#docsContent').innerHTML=projectDocs.map(d=>`<article class="doc-card"><div class="doc-icon">📄</div><h3>${esc(d.title)}</h3><p>${esc(d.desc)}</p><a class="secondary doc-link" href="${d.file}" target="_blank" rel="noopener">Dokument öffnen</a></article>`).join('');
  TelemetryCore.event('project_docs_opened');
}
function closeDocs(){docsModal.hidden=true;document.body.classList.remove('modal-open')}


function dashboardStats(){const sum=TelemetryCore.summary();const ready=Object.entries(modules).filter(([,m])=>m.ready);const completed=ready.filter(([id])=>state.completed[id]).length;const totalAnswers=sum.totalCorrect+sum.totalWrong;const pct=totalAnswers?Math.round(sum.totalCorrect/totalAnswers*100):0;const points=Object.values(state.completed).reduce((n,x)=>n+(Number(x.points)||0),0);return {sum,ready,completed,totalAnswers,pct,points}}
function openDashboard(){stopSpeech();const {sum,ready,completed,totalAnswers,pct,points}=dashboardStats();dashboardModal.hidden=false;document.body.classList.add('modal-open');const moduleRows=ready.map(([id,m])=>{const x=sum.byModule[id]||{};const result=x.answers?Math.round((x.correct||0)/x.answers*100):(state.completed[id]?Math.round((state.completed[id].score||0)/Math.max(1,state.completed[id].total||1)*100):0);return {id,title:m.title,result,runs:x.runs||0,minutes:x.minutes||0,completed:Boolean(state.completed[id])}});const donutCorrect=totalAnswers?pct:0;const finishes=sum.finishes.slice(-10);const plot=finishes.map((f,i)=>({label:`D${i+1}`,value:Number(f.total)?Math.max(0,Math.min(100,Math.round(Number(f.score||0)/Number(f.total)*100))):0,module:modules[f.module]?.title||'Modul'}));const w=420,h=180,padL=42,padR=16,padT=18,padB=38;const x=(i)=>plot.length<2?padL+(w-padL-padR)/2:padL+i*(w-padL-padR)/(plot.length-1);const y=v=>padT+(100-v)*(h-padT-padB)/100;const pointsLine=plot.map((d,i)=>`${x(i)},${y(d.value)}`).join(' ');const maxMinutes=Math.max(1,...moduleRows.map(r=>Number(r.minutes)||0));document.querySelector('#dashboardContent').innerHTML=`<div class="dashboard-kpis"><div><b>${completed}/${ready.length}</b><span>Module abgeschlossen</span></div><div><b>${pct}%</b><span>Richtige Antworten</span></div><div><b>${sum.totalMinutes} Min.</b><span>Lernzeit</span></div><div><b>${points}</b><span>Academy-Punkte</span></div></div><div class="dashboard-grid"><section class="dashboard-card"><h3>Fortschritt nach Themen</h3>${moduleRows.map(r=>`<div class="bar-row"><span>${esc(r.title)}</span><div><i style="width:${r.result}%"></i></div><b>${r.result}%</b></div>`).join('')||'<p>Noch keine Daten.</p>'}</section><section class="dashboard-card"><h3>Antworten</h3><div class="donut" style="--value:${donutCorrect}"><span>${pct}%<small>richtig</small></span></div><p>Richtig: <b>${sum.totalCorrect}</b> · Falsch: <b>${sum.totalWrong}</b></p></section><section class="dashboard-card chart-card"><h3>Lernzeit je Modul</h3><div class="column-chart">${moduleRows.map(r=>`<div title="${esc(r.title)}: ${r.minutes||0} Minuten"><i style="height:${Math.max(8,Math.round((Number(r.minutes)||0)/maxMinutes*112))}px"></i><span>${r.minutes||0}m</span><small>${esc(r.title)}</small></div>`).join('')}</div></section><section class="dashboard-card chart-card"><h3>Lernentwicklung</h3><p class="chart-explain">Richtige Antworten je abgeschlossenem Durchgang. Je höher der Punkt, desto sicherer das Ergebnis.</p><svg class="line-chart meaningful" viewBox="0 0 ${w} ${h}" role="img" aria-label="Lernentwicklung in Prozent"><g class="chart-grid"><line x1="${padL}" y1="${y(100)}" x2="${w-padR}" y2="${y(100)}"/><line x1="${padL}" y1="${y(75)}" x2="${w-padR}" y2="${y(75)}"/><line x1="${padL}" y1="${y(50)}" x2="${w-padR}" y2="${y(50)}"/><line x1="${padL}" y1="${y(25)}" x2="${w-padR}" y2="${y(25)}"/></g><g class="chart-labels"><text x="5" y="${y(100)+4}">100%</text><text x="12" y="${y(75)+4}">75%</text><text x="12" y="${y(50)+4}">50%</text><text x="12" y="${y(25)+4}">25%</text></g>${plot.length?`<polyline points="${pointsLine}" fill="none" stroke="currentColor" stroke-width="5" stroke-linecap="round" stroke-linejoin="round"/>${plot.map((d,i)=>`<circle cx="${x(i)}" cy="${y(d.value)}" r="6"><title>${esc(d.module)} · Durchgang ${i+1}: ${d.value}%</title></circle><text x="${x(i)}" y="${h-12}" text-anchor="middle">${i+1}</text>`).join('')}`:`<text x="${w/2}" y="${h/2}" text-anchor="middle">Noch keine abgeschlossenen Durchgänge</text>`}</svg><div class="chart-legend"><span>Y-Achse: richtige Antworten</span><span>X-Achse: Durchgänge</span></div><p>${plot.length?`Letzter Durchgang: <b>${plot.at(-1).value}%</b>. ${plot.length>1?`Veränderung seit dem ersten Durchgang: <b>${plot.at(-1).value-plot[0].value>=0?'+':''}${plot.at(-1).value-plot[0].value} Prozentpunkte</b>.`:''}`:'Starte dein erstes Modul, damit hier deine Entwicklung erscheint.'}</p></section></div><div class="dashboard-recommendation"><h3>Persönliche Empfehlung</h3><p>${pct>=80?'Sehr gut. Vertiefe als Nächstes ein weiteres Thema.':'Wiederhole Themen mit weniger als 80 Prozent, um dein Wissen zu festigen.'}</p></div>`;TelemetryCore.event('dashboard_opened')}
function closeDashboard(){dashboardModal.hidden=true;document.body.classList.remove('modal-open')}


let volumeHideTimer=null;
function syncVolumeUI(){['headerVolume','modalVolume'].forEach(id=>{const e=document.querySelector('#'+id);if(e)e.value=state.volume});['headerVolumeValue','modalVolumeValue'].forEach(id=>{const e=document.querySelector('#'+id);if(e)e.textContent=`${state.volume}/10`})}
function showVolumePopover(){const p=document.querySelector('#volumePopover');if(!p||!state.sound)return;clearTimeout(volumeHideTimer);p.hidden=false;syncVolumeUI()}
function scheduleVolumeHide(){clearTimeout(volumeHideTimer);volumeHideTimer=setTimeout(()=>{const p=document.querySelector('#volumePopover');if(p)p.hidden=true},950)}
function setUserVolume(v){state.volume=Math.max(1,Math.min(10,Number(v)||7));syncVolumeUI();if(VoiceCore.activeAudio)VoiceCore.activeAudio.volume=state.volume/10;if(VoiceCore.activeUtterance)VoiceCore.activeUtterance.volume=Math.max(.1,state.volume/10);saveSettings()}

backBtn.onclick=goBack;pauseBtn.onclick=togglePause;repeatBtn.onclick=()=>state.lastSpeech&&speak(state.lastSpeech,{characterKey:currentSpeakerKey()});soundBtn.onclick=()=>{state.sound=!state.sound;soundBtn.textContent=state.sound?'🔊':'🔇';saveSettings();if(!state.sound){stopSpeech();setVoiceMonitor('off','Ton aus');document.querySelector('#volumePopover').hidden=true}else{unlockAcademyAudio();setVoiceMonitor('ready','Ton bereit');showVolumePopover()}};soundBtn.textContent=state.sound?'🔊':'🔇';
const soundWrap=document.querySelector('.sound-control-wrap'),volumePopover=document.querySelector('#volumePopover'),headerVolume=document.querySelector('#headerVolume');
if(soundWrap){soundWrap.addEventListener('mouseenter',()=>state.sound&&showVolumePopover());soundWrap.addEventListener('mouseleave',scheduleVolumeHide);soundWrap.addEventListener('focusin',()=>state.sound&&showVolumePopover());soundWrap.addEventListener('focusout',scheduleVolumeHide)}
if(volumePopover){volumePopover.addEventListener('mouseenter',()=>clearTimeout(volumeHideTimer));volumePopover.addEventListener('mouseleave',scheduleVolumeHide)}
if(headerVolume)headerVolume.oninput=e=>setUserVolume(e.target.value);
document.addEventListener('pointerdown',e=>{if(volumePopover&&!volumePopover.hidden&&!e.target.closest('.sound-control-wrap'))volumePopover.hidden=true});
dashboardBtn.onclick=openDashboard;roadmapBtn.onclick=openRoadmap;if(docsBtn)docsBtn.onclick=openDocs;settingsBtn.onclick=openSettings;exitBtn.onclick=openExit;document.querySelector('#settingsClose').onclick=closeSettings;document.querySelector('#adminPinCancel').onclick=closeAdminPin;document.querySelector('#adminPinConfirm').onclick=verifyAdminPin;document.querySelector('#adminPinInput').onkeydown=e=>{if(e.key==='Enter')verifyAdminPin()};document.querySelector('#participantDifferent').onclick=chooseDifferentParticipantName;document.querySelector('#participantSame').onclick=showRecoveryEntry;document.querySelector('#participantClaim').onclick=claimExistingParticipant;document.querySelector('#participantChoiceCancel').onclick=closeParticipantChoice;document.querySelector('#participantCodeContinue').onclick=closeParticipantCode;document.querySelector('#participantCodeCopy').onclick=()=>navigator.clipboard?.writeText(document.querySelector('#participantCodeValue').textContent);document.querySelector('#episodeEditorClose').onclick=closeEpisodeEditor;document.querySelector('#dashboardClose').onclick=closeDashboard;document.querySelector('#roadmapClose').onclick=closeRoadmap;document.querySelector('#docsClose').onclick=closeDocs;document.querySelector('#exitCancel').onclick=closeExit;document.querySelector('#exitConfirm').onclick=confirmExit;document.querySelector('#diagnosticsClose').onclick=closeDiagnostics;document.querySelector('#diagnosticsRun').onclick=async()=>{const btn=document.querySelector('#diagnosticsRun');const old=btn.textContent;btn.disabled=true;btn.textContent='Systemprüfung läuft …';const host=document.querySelector('#diagnosticsContent');try{const r=await window.KCDatabaseSecurityCore.quickSystemCheck();renderDiagnostics();host.insertAdjacentHTML('beforeend',`<article class="diagnostics-result ${r.ok?'status-ok':'status-warn'}"><b>Ergebnis</b><span>${r.score} %</span><small>${r.ok?'Alles Wichtige ist in Ordnung.':'Es wurden Hinweise gefunden. Bitte Supportbericht senden.'}</small></article>`);document.querySelector('#diagnosticsSupportExport').hidden=false;document.querySelector('#diagnosticsSupportMail').hidden=false;flashLed(indexedTrafficLed);flashLed(supabaseTrafficLed)}catch(e){host.insertAdjacentHTML('beforeend',`<article><b>Systemprüfung fehlgeschlagen</b><span class="status-warn">Fehler</span><small>${esc(e.message)}</small></article>`);document.querySelector('#diagnosticsSupportExport').hidden=false;document.querySelector('#diagnosticsSupportMail').hidden=false}finally{btn.disabled=false;btn.textContent=old}};document.querySelector('#diagnosticsSupportExport').onclick=()=>window.KCDatabaseSecurityCore?.exportSupportReport?.();document.querySelector('#diagnosticsSupportMail').onclick=()=>window.KCDatabaseSecurityCore?.openSupportEmail?.();document.querySelector('.db-status-cluster')?.addEventListener('dblclick',openDiagnostics);
document.querySelectorAll('.settings-tab').forEach(btn=>btn.onclick=()=>{document.querySelectorAll('.settings-tab').forEach(x=>x.classList.toggle('active',x===btn));document.querySelectorAll('.settings-pane').forEach(x=>x.classList.toggle('active',x.dataset.pane===btn.dataset.tab));if(btn.dataset.tab==='voice')refreshVoiceStudio();if(btn.dataset.tab==='character')refreshCharacterStudio()});
document.querySelector('#modalCoach').onchange=e=>{state.coach=e.target.value;saveSettings()};document.querySelector('#modalAddress').onchange=e=>{state.address=e.target.value;saveSettings()};document.querySelector('#modalRate').oninput=e=>{state.speechRate=Number(e.target.value)||1;saveSettings()};document.querySelector('#modalSubtitles').onchange=e=>{state.subtitles=e.target.checked;saveSettings()};document.querySelector('#modalSound').onchange=e=>{state.sound=e.target.checked;soundBtn.textContent=state.sound?'🔊':'🔇';saveSettings();if(!state.sound)stopSpeech();else{unlockAcademyAudio();setVoiceMonitor('ready','Ton bereit')}};document.querySelector('#voiceProvider').onchange=e=>{state.voiceProvider=e.target.value;saveSettings();window.KCVoiceProvider?.configure({...elevenConfig,provider:state.voiceProvider});const panel=document.querySelector('#elevenLabsPanel');if(panel)panel.hidden=!['elevenlabs','openai','azure'].includes(state.voiceProvider);refreshVoiceStudio()};document.querySelector('#voiceExportConfig').onclick=exportVoiceConfig;document.querySelector('#elevenSaveConfig').onclick=saveElevenForm;document.querySelector('#elevenTest').onclick=testElevenConnection;document.querySelector('#elevenGenerateCurrent').onclick=generateCurrentVoice;document.querySelector('#elevenGenerateAll').onclick=generateAllStaleVoices;document.querySelector('#elevenVoiceId').onchange=saveElevenForm;document.querySelector('#elevenModel').onchange=saveElevenForm;document.querySelector('#providerOutputFormat').onchange=saveElevenForm;document.querySelector('#providerInstructions').onchange=saveElevenForm;document.querySelector('#elevenAutoGenerate').onchange=saveElevenForm;document.querySelector('#runAudioTuv').onclick=runAcademyAudioTuv;document.querySelector('#designFontScale').oninput=e=>{state.fontScale=Number(e.target.value)||1;applyTheme(state.theme)};document.querySelector('#designReduceMotion').onchange=e=>{state.reduceMotion=e.target.checked;applyTheme(state.theme)};document.querySelector('#characterStudioSave').onclick=saveCharacterProfile;document.querySelector('#characterStudioReset').onclick=resetCharacterProfile;document.querySelector('#characterStudioPreview').onclick=()=>speak(chars[characterStudioKey].intro,{characterKey:characterStudioKey});
document.querySelector('#voiceCharacter').onchange=refreshVoiceStudio;document.querySelector('#voiceDialog').onchange=refreshVoiceStudio;
document.querySelector('#recordStart').onclick=startRecording;document.querySelector('#recordStop').onclick=stopRecording;document.querySelector('#recordPlay').onclick=playRecording;document.querySelector('#recordSave').onclick=saveRecording;document.querySelector('#recordDelete').onclick=deleteRecording;document.querySelector('#audioUpload').onchange=e=>importAudio(e.target.files?.[0]);document.querySelectorAll('.theme-preview').forEach(b=>b.onclick=()=>applyTheme(b.dataset.theme));
document.addEventListener('pointerdown',()=>{unlockAcademyAudio();primeAcademySpeech()},{passive:true});document.addEventListener('keydown',()=>{unlockAcademyAudio();primeAcademySpeech()});
function applyNativeTooltips(root=document){
  root.querySelectorAll('button,[role=button],input,select').forEach(el=>{if(!el.title){const label=el.getAttribute('aria-label')||el.dataset.tooltip;if(label)el.title=label}});
}
