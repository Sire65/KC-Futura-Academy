...
const cryptoAdapter={
 async encryptJson(value,meta={}){const s=secure();if(!s?.encryptBytes)throw new Error('KC Secure Storage nicht bereit');await s.ready;const context=`KARRIERELEITER-${String(meta.purpose||'data').toUpperCase().replace(/[^A-Z0-9:_-]/g,'-')}`;return {__kc_futura_secure:1,...await s.encryptBytes(enc.encode(JSON.stringify(value)),context)}},
 async decryptJson(envelope){if(envelope==null)return envelope;if(!envelope?.cipher)return envelope;const s=secure();if(!s?.decryptBytes)throw new Error('KC Secure Storage nicht bereit');await s.ready;const buf=await s.decryptBytes(envelope);return JSON.parse(dec.decode(buf));}
};
const speechAdapter={
 speak(text,opts={}){if(window.KCVoiceProvider?.speak){try{return window.KCVoiceProvider.speak(text,opts)}catch{}}if(!('speechSynthesis'in window)){opts.onend?.();return Promise.resolve()}return new Promise(resolve=>{try{speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(String(text||''));u.lang=opts.lang||'de-DE';u.rate=1;u.onend=()=>{opts.onend?.();resolve()};u.onerror=()=>{opts.onend?.();resolve()};speechSynthesis.speak(u)}catch{opts.onend?.();resolve()}})},
...
async function context(){const p=await profile();return {moduleId:MODULE_ID,version:VERSION,releaseHash:BUILD_HASH,user:{id:p.id,name:p.name,professions:p.professions,roles:roles()},profile:p,roles:roles(),cryptoAdapter,speechAdapter,learningStore,exitModule:closeModule}}
...
function makeOverlay(){if(overlay)return overlay;ensureStyle();overlay=document.createElement('div');overlay.className='kc-kl-overlay';overlay.hidden=true;overlay.innerHTML=`<div class="kc-kl-bar"><b>🎓 KC FUTURA · Karriereleiter</b><span>Build ${VERSION} · ${BUILD_HASH}</span><button type="button" data-kl-close>Zur Academy</button></div><iframe class="kc-kl-frame" title="KC FUTURA Karriereleiter" allow="autoplay"></iframe>`;document.body.appendChild(overlay);overlay.querySelector('[data-kl-close]').onclick=closeModule;return overlay}
async function openModule(){const o=makeOverlay(),frame=o.querySelector('iframe');window.KCFuturaModuleContext=await context();o.hidden=false;document.body.classList.add('modal-open');frame.src=`${MODULE_URL}?futura=1&v=${encodeURIComponent(VERSION)}&h=${BUILD_HASH}&t=${Date.now()}`;try{speechAdapter.stop()}catch{}}
...
function injectCard(){const home=document.querySelector('#screen .academy-home');if(!home||home.querySelector('[data-karriereleiter-launch]'))return;ensureStyle();const card=document.createElement('section');card.className='kc-kl-launch';card.dataset.karriereleiterLaunch='1';card.innerHTML=`<div class="kc-kl-launch-icon">🎓</div><div><h3>Karriereleiter · Fachwissen & Prüfung</h3><p>400 Fachfragen, Prüfungsvorbereitung, Lernkarten, Mein Buch sowie Puzzle- und QUICK-Aufgaben.</p><small>Build ${VERSION} · ${BUILD_HASH} · verschlüsselter Lernstand</small></div><button type="button">Karriereleiter starten</button>`;card.querySelector('button').onclick=openModule;const intro=home.querySelector('.home-intro');(intro||home.firstElementChild)?.insertAdjacentElement('afterend',card)}
function injectTopButton(){const host=document.querySelector('.top-actions');if(!host||host.querySelector('[data-karriereleiter-top]'))return;const b=document.createElement('button');b.type='button';b.className='icon-btn';b.dataset.karriereleiterTop='1';b.title=`Karriereleiter ${VERSION} öffnen`;b.setAttribute('aria-label','Karriereleiter öffnen');b.textContent='🎓';b.onclick=openModule;const dash=document.querySelector('#dashboardBtn');host.insertBefore(b,dash||host.firstChild)}
...
window.KCFuturaKarriereleiter={version:VERSION,releaseHash:BUILD_HASH,open:openModule,close:closeModule,context};
scan();
})();