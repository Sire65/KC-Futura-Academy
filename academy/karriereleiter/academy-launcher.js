...
async function openModule(){
  const o=makeOverlay(),frame=o.querySelector('iframe');
  window.KCFuturaModuleContext=await context();
  o.hidden=false;document.body.classList.add('modal-open');
  frame.src=`${MODULE_URL}?futura=1&v=${encodeURIComponent(VERSION)}&t=${Date.now()}`;
  try{speechAdapter.stop()}catch{}
}
...
function injectCard(){
  const home=document.querySelector('#screen .academy-home');if(!home||home.querySelector('[data-karriereleiter-launch]'))return;
  ensureStyle();const card=document.createElement('section');card.className='kc-kl-launch';card.dataset.karriereleiterLaunch='1';card.innerHTML=`<div class="kc-kl-launch-icon">🎓</div><div><h3>Karriereleiter · Fachwissen & Prüfung</h3><p>Vom Praktikanten bis zum Profi: 400 Fachfragen, Prüfungsvorbereitung, adaptive Lernkarten, Fleisch-, Fisch-, Kräuter- und Gewürzkunde.</p><small>Eigener Lernbereich · Fortschritt bleibt verschlüsselt gespeichert</small></div><button type="button">Karriereleiter starten</button>`;card.querySelector('button').onclick=openModule;const intro=home.querySelector('.home-intro');(intro||home.firstElementChild)?.insertAdjacentElement('afterend',card);
}
...
window.KCFuturaKarriereleiter={version:VERSION,open:openModule,close:closeModule,context};
scan();
})();
